...
.. list-table::
   ...
   * - Config key
     - Python equivalent
     - Description
   * - ``sigma``, ``ls``, ``ls_factor``, ``param_scheme``, ``n_landmarks``, ``batch_size``, ``eps``, ``jit_compile``, ``random_state``, ``use_empirical_variance``
     - ``gp=GPSettings(...)``
     - GP model parameters. In DA, ``sigma`` is the density estimator's and ``eps`` and ``use_empirical_variance`` do not apply. ``param_scheme`` defaults to ``condition1`` in DE and ``separate`` in DA
