...
class TestCleanupSmooth:
    def test_cleanup_keeps_smoothed_by_default(self, adata_with_smooth):
        ...
        ad = adata_with_smooth
        assert any("kompot_smooth" in k for k in ad.layers)

        cleanup(ad, analysis_type="smooth")

        # Imputed layers should be kept
        assert any("_smoothed" in k for k in ad.layers if "kompot_smooth" in k)
        # Std and obs_variance layers should be removed
        assert not any("_std" in k for k in ad.layers if "kompot_smooth" in k)
        assert not any("_obs_variance" in k for k in ad.layers if "kompot_smooth" in k)
    ...
    def test_cleanup_removes_all_with_explicit_false(self, adata_with_smooth):
        ...
        remaining = [k for k in ad.layers if "kompot_smooth" in k]
    ...
    def test_cleanup_single_run(self, adata_with_smooth):
        ...
        remaining_a = [k for k in ad.layers if "kompot_smooth_A" in k]
        assert len(remaining_a) >= 2

    def test_cleanup_keep_layers_true(self, adata_with_smooth):
        ad = adata_with_smooth
        n_before = sum(1 for k in ad.layers if "kompot_smooth" in k)
        cleanup(ad, analysis_type="smooth", keep_layers=True)
        n_after = sum(1 for k in ad.layers if "kompot_smooth" in k)
