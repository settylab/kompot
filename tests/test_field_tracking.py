...
from kompot.anndata.utils import validate_field_run_id, get_run_from_history
...
class TestFieldTrackingAndValidation:
    ...
    def test_field_tracking_basic(self, dummy_adata, caplog):
        ...
        assert isinstance(tracking, dict), (
            f"Expected tracking to be a dict, but got {type(tracking)}"
        )
    ...
    def test_validate_field_run_id(self, dummy_adata):
        ...
        dummy_adata.uns["kompot_de"]["anndata_fields"] = {"var": {"test_field": 1}}
        ...
        valid, actual_run_id, message = validate_field_run_id(
            dummy_adata, "nonexistent_field", "var", 1, "kompot_de"
        )
    ...
    def test_get_run_from_history_with_validation(self, dummy_adata, caplog):
        ...
        print(
            f"Available fields in anndata_fields['obs']: {list(tracking['obs'].keys())}"
        )
        ...
        assert lfc_key in tracking["obs"], (
            f"Expected {lfc_key} to be in tracking['obs'], but only found: {list(tracking['obs'].keys())}"
        )
        ...
        assert actual_run_id == 999, (
            f"Expected actual_run_id to be 999, got {actual_run_id}"
        )
        assert message is not None, "Expected a warning message"
        assert "written by run_id=999" in message, (
            f"Expected message to mention run_id=999, got {message}"
        )
    ...
    def test_infer_da_keys_with_validation(self, dummy_adata, caplog):
        ...
        # Make sure ptp_key is in the tracking
        assert ptp_key in tracking["obs"], (
            f"Expected {ptp_key} to be in tracking['obs'], but only found: {list(tracking['obs'].keys())}"
        )

        # Now modify the tracking to simulate a different run writing to the ptp field
        tracking["obs"][ptp_key] = 999
        ...
        caplog.clear()
        inferred_lfc_key, inferred_ptp_key, thresholds = _infer_da_keys(
            dummy_adata, run_id=-1
        )

        # Verify the keys were inferred correctly
        assert inferred_lfc_key == lfc_key
        assert inferred_ptp_key == ptp_key
