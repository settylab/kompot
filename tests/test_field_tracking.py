...
class TestFieldTrackingAndValidation:
    ...
    def test_infer_da_keys_with_validation(self, dummy_adata, caplog):
        ...
        assert ptp_key in tracking['obs'], f"Expected {ptp_key} to be in tracking['obs'], but only found: {list(tracking['obs'].keys())}"
        ...
        caplog.clear()
        inferred_lfc_key, inferred_ptp_key, thresholds = _infer_da_keys(dummy_adata, run_id=-1)
