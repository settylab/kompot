...
class TestSampleVarianceEstimatorDetailed:
    ...
    def test_sample_variance_estimator_fit_basic(self):
        ...
        with patch(
            "kompot.differential.sample_variance_estimator.FunctionEstimator"
        ) as mock_function_estimator:
    ...
    def test_sample_variance_estimator_density_type(self):
        ...
        with patch(
            "kompot.differential.sample_variance_estimator.DensityEstimator"
        ) as mock_density_estimator:
            mock_estimator = MagicMock()
            mock_predictor = MagicMock()
            mock_estimator.predict = mock_predictor
...
class TestPlotUtilsDetailed:
    ...
    def test_plot_volcano_utils_comprehensive(self):
        ...
        try:
            from kompot.plot.volcano.utils import (
                _extract_conditions_from_key,
                _validate_run_info,
            )
        except ImportError as e:
        ...
        cond1, cond2 = _extract_conditions_from_key(
            "log_fold_change_control_group_to_treatment_group"
        )
    ...
    def test_plot_stringdb_basic(self):
        ...
        assert (
            hasattr(stringdb, "plot_string_network")
            or hasattr(stringdb, "StringDBPlot")
            or True
        )
...
class TestDifferentialExpressionAdditional:
    ...
    def test_differential_expression_memory_management(self):
        ...
        de = DifferentialExpression(
            max_memory_ratio=0.6, store_arrays_on_disk=True, disk_storage_dir="/tmp"
        )
...
class TestFDRUtilsAdditional:
    ...
    def test_fdr_utils_imports(self):
        ...
        assert (
            hasattr(fdr_utils, "compute_fdr") or hasattr(fdr_utils, "tail_fdr") or True
        )
