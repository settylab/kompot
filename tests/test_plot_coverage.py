...
def create_plot_test_adata(n_cells=50, n_genes=20):
    ...
    obs = pd.DataFrame({"group": groups, "continuous_var": np.random.rand(n_cells)})
...
class TestVolcanoPlotsCoverage:
    ...
    def test_volcano_da_basic(self, plot_adata):
        ...
        plot_adata.obs["log_fold_change_A_to_B"] = np.random.normal(
            0, 1, plot_adata.n_obs
        )
        plot_adata.obs["ptp_A_to_B"] = np.random.uniform(0, 5, plot_adata.n_obs)
        plot_adata.obs["direction_A_to_B"] = np.random.choice(
            ["up", "down", "none"], plot_adata.n_obs
        )
...
class TestEmbeddingPlotsCoverage:
    ...
    def test_embedding_basic(self, plot_adata):
        ...
        with patch("matplotlib.pyplot.show"):
            result = embedding(plot_adata, basis="umap", groups="group")
    ...
    def test_embedding_custom_parameters(self, plot_adata):
        ...
        with patch("matplotlib.pyplot.show"):
            result = embedding(
                plot_adata, basis="umap", groups="group", background_color="lightblue"
            )
    ...
    def test_embedding_dm_eigenvectors(self, plot_adata):
        ...
        with patch("matplotlib.pyplot.show"):
            result = embedding(plot_adata, basis="DM_EigenVectors", groups="group")
...
class TestPlotUtilsCoverage:
    ...
    def test_extract_conditions_from_key(self):
        ...
        cond1, cond2 = _extract_conditions_from_key(
            "some_key_condition_1_to_condition_2"
        )
