...
def test_dask_vs_no_dask_comparison():
    ...
    def run_with_disk_backing(temp_dir, use_dask):
        ...
        try:
            ...
            small_X = X[
                :20
            ]  # Use only 20 cells for full covariance to speed up CI tests
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        diag_abs_diff = np.abs(
            results["with_dask"]["diag"] - results["without_dask"]["diag"]
        )
        ...
        logger.info(
            f"Diagonal result differences - Max: {diag_max_diff}, Mean: {diag_mean_diff}"
        )
        logger.info(
            f"Covariance result differences - Max: {cov_max_diff}, Mean: {cov_mean_diff}"
        )

        # Test that the results are almost identical
        assert np.all(np.isfinite(results["with_dask"]["diag"])), (
            "Dask-based diagonal results contain NaN or Inf"
        )
        assert np.all(np.isfinite(results["without_dask"]["diag"])), (
            "Non-Dask diagonal results contain NaN or Inf"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Timings with Dask: fit={dask_timing['fit']:.4f}s, predict={dask_timing['predict']:.4f}s, cov={dask_timing['cov_predict']:.4f}s"
    )
    print(
        f"Timings without Dask: fit={no_dask_timing['fit']:.4f}s, predict={no_dask_timing['predict']:.4f}s, cov={no_dask_timing['cov_predict']:.4f}s"
    )
    ...
    print("Performance comparison (Dask vs. no Dask):")
    print(f"  - Fit: {fit_ratio:.2f}x {'slower' if fit_ratio > 1 else 'faster'}")
    print(
        f"  - Predict: {predict_ratio:.2f}x {'slower' if predict_ratio > 1 else 'faster'}"
    )
