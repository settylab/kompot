...
def test_sample_col_parameter():
    ...
    for field in field_mapping["obs"].keys():
        if "lfc" in field and "lfc_direction" not in field:
            lfc_field = field
            break
    ...
    assert (
        last_run_info_no_samples["params"]["sample_col"] is None
    )  # no sample variance
    ...
    np.testing.assert_allclose(
        ...
        rtol=1e-3,
        atol=1e-3,  # More tolerant comparison for faster tests
    )
    ...
    zscore_key_with_samples = "test_sample_col_A_to_B_lfc_zscore_sample_var"
    zscore_key_no_samples = "test_no_sample_col_A_to_B_lfc_zscore"
    ...
    X_test = adata.obsm["DM_EigenVectors"][
        :10
    ]  # Just use 10 test points for faster testing
    ...
    assert np.mean(sample_variance1) > 0, (
        "Sample variance for condition 1 should be greater than zero"
    )
    assert np.mean(sample_variance2) > 0, (
        "Sample variance for condition 2 should be greater than zero"
    )
...
def test_generate_output_field_names():
    ...
    assert not de_fields["mean_lfc_key"].endswith(
        "_sample_var"
    )  # This field isn't affected
...
def test_compute_differential_expression_warns_overwrite(mock_warning):
    ...
    args, _ = mock_warning.call_args
    assert (
        "Differential expression results with result_key='test_key' already exist"
        in args[0]
    )
    assert "Set overwrite=" in args[0]
...
def test_landmark_reuse_and_storage():
    ...
    assert "store_landmarks_run" in adata.uns, (
        "Result key should be created in adata.uns"
    )
    assert "landmarks" in adata.uns["store_landmarks_run"], (
        "Landmarks should be stored in result key"
    )
    ...
    assert landmarks_shape[1] == 5, (
        f"Expected 5 dimensions, got {landmarks_shape[1]}"
    )  # DM_EigenVectors default dimension
    ...
    if (
        "store_landmarks_run" in adata.uns
        and "landmarks" in adata.uns["store_landmarks_run"]
    ):
...
def test_landmark_cross_analysis_search():
    ...
    assert (
        adata.uns["custom_landmark_search"]["landmarks"].shape
        == adata.uns["kompot_custom"]["landmarks"].shape
    )


@pytest.mark.skip(
    reason="Disk backed options are tested in test_mahalanobis_approaches"
)
def test_disk_backed_options():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert (
            "store_arrays_on_disk" in adata.uns["kompot_da"]["last_run_info"]["params"]
        )
        assert (
            adata.uns["kompot_da"]["last_run_info"]["params"]["store_arrays_on_disk"]
            is True
        )
        assert "disk_storage_dir" in adata.uns["kompot_da"]["last_run_info"]["params"]
        assert (
            adata.uns["kompot_da"]["last_run_info"]["params"]["disk_storage_dir"]
            == temp_dir
        )
        assert "max_memory_ratio" in adata.uns["kompot_da"]["last_run_info"]["params"]
        assert (
            adata.uns["kompot_da"]["last_run_info"]["params"]["max_memory_ratio"] == 0.7
        )
        ...
        assert (
            "store_arrays_on_disk" in adata.uns["kompot_de"]["last_run_info"]["params"]
        )
        assert (
            adata.uns["kompot_de"]["last_run_info"]["params"]["store_arrays_on_disk"]
            is True
        )
        assert "disk_storage_dir" in adata.uns["kompot_de"]["last_run_info"]["params"]
        assert (
            adata.uns["kompot_de"]["last_run_info"]["params"]["disk_storage_dir"]
            == temp_dir
        )
...
class TestRunInfo:
    ...
    def test_runinfo_field_tracking(self):
        ...
        assert len(uns_fields) > 0 or "test_field_tracking" in adata.uns, (
            "No uns fields were created"
        )
        ...
        obs_fields_after_second_run = [
            col for col in adata.obs.columns if "test_field_tracking" in col
        ]
        assert len(obs_fields_after_second_run) > 0, (
            "No observation fields after second run"
        )
    ...
    def test_runcomparison_overwritten_fields(self):
        ...
        if first_run_timestamp:
            if (
                "overwrite_run1" in adata.uns
                and "timestamp" in adata.uns["overwrite_run1"]
            ):
                second_run_timestamp = adata.uns["overwrite_run1"].get("timestamp")
                assert second_run_timestamp != first_run_timestamp, (
                    "Run timestamp didn't change after rerunning"
                )
...
def test_gene_subset_order_preservation():
    ...
    mean_lfc_column = "test_all_genes_shuffled_A_to_B_mean_lfc"
    ...
    mean_lfc_column_subset = "test_gene_subset_A_to_B_mean_lfc"
