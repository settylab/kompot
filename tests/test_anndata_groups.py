...
def create_test_anndata(
    n_cells=60, n_genes=20, with_sample_col=False, with_multiple_groups=False
):
...
def test_parse_groups_dict():
    ...
    subset_masks, subset_names = parse_groups(
        adata, {"category": ["cat1", "cat2"], "is_selected": True}
    )
    assert len(subset_masks) == 1
...
def test_compute_de_with_groups_string():
    ...
    for category in categories:
        ...
        assert category in varm_df.columns, (
            f"Category '{category}' not found in varm matrix columns: {list(varm_df.columns)}"
        )

        # Check that the column has values (not all NaN)
        assert not pd.isna(varm_df[category]).all(), (
            f"All values for category '{category}' are NaN"
        )
...
def test_compute_de_with_groups_dict():
    ...
    assert matching_cols, (
        f"No column for subset '{expected_subset}' found in varm matrix columns: {list(varm_df.columns)}"
    )
    subset_col = matching_cols[0]

    # Check that the column has values (not all NaN)
    assert not pd.isna(varm_df[subset_col]).all(), (
        f"All values for subset '{subset_col}' are NaN"
    )
...
def test_compute_de_with_named_groups():
    ...
    # Get the varm keys for group metrics using our helper
    mean_lfc_varm_key, mahalanobis_varm_key = check_group_metrics_varm(
        adata, "de_test_named"
    )
    ...
    print(
        f"Found varm keys - mean LFC: {mean_lfc_varm_key}, mahalanobis: {mahalanobis_varm_key}"
    )

    # Check that the varm matrices exist
    assert mean_lfc_varm_key in adata.varm, (
        f"varm key {mean_lfc_varm_key} not found in adata.varm"
    )
    assert mahalanobis_varm_key in adata.varm, (
        f"varm key {mahalanobis_varm_key} not found in adata.varm"
    )
    ...
    for group_name in named_filters.keys():
        ...
        assert mean_lfc_cols, (
            f"No column for named group '{group_name}' found in mean LFC matrix: {list(mean_lfc_df.columns)}"
        )
        mean_lfc_col = mean_lfc_cols[0]
        assert not pd.isna(mean_lfc_df[mean_lfc_col]).all(), (
            f"All values for named group '{group_name}' in mean LFC matrix are NaN"
        )

        # Check in mahalanobis varm
        mahalanobis_cols = [
            col for col in mahalanobis_df.columns if group_name in str(col)
        ]
        assert mahalanobis_cols, (
            f"No column for named group '{group_name}' found in mahalanobis matrix: {list(mahalanobis_df.columns)}"
        )
        mahalanobis_col = mahalanobis_cols[0]
        assert not pd.isna(mahalanobis_df[mahalanobis_col]).all(), (
            f"All values for named group '{group_name}' in mahalanobis matrix are NaN"
        )
...
def test_compute_de_with_multiple_groups():
    ...
    filters = [{"category": "cat1"}, {"category": "cat2", "is_selected": True}]
    ...
    # Check that the varm matrices exist
    assert mean_lfc_key in adata.varm, (
        f"varm key {mean_lfc_key} not found in adata.varm"
    )
    assert mahalanobis_key in adata.varm, (
        f"varm key {mahalanobis_key} not found in adata.varm"
    )
    ...
    assert cat1_subset_cols, (
        f"No column for 'cat1' found in mean LFC matrix: {list(mean_lfc_df.columns)}"
    )
    ...
    cat2_subset_cols = [
        col
        for col in mean_lfc_df.columns
        if "cat2" in str(col) and "is_selected" in str(col)
    ]
    assert cat2_subset_cols, (
        f"No column for 'cat2,is_selected' found in mean LFC matrix: {list(mean_lfc_df.columns)}"
    )
    cat2_col = cat2_subset_cols[0]

    # Check that the columns have values (not all NaN)
    assert not pd.isna(mean_lfc_df[cat1_col]).all(), (
        f"All values for '{cat1_col}' in mean LFC matrix are NaN"
    )
    assert not pd.isna(mean_lfc_df[cat2_col]).all(), (
        f"All values for '{cat2_col}' in mean LFC matrix are NaN"
    )

    # Check that mahalanobis columns exist and have values
    mahalanobis_cat1_cols = [
        col for col in mahalanobis_df.columns if "cat1" in str(col)
    ]
    assert mahalanobis_cat1_cols, (
        f"No column for 'cat1' found in mahalanobis matrix: {list(mahalanobis_df.columns)}"
    )
    mahalanobis_cat1_col = mahalanobis_cat1_cols[0]

    mahalanobis_cat2_cols = [
        col
        for col in mahalanobis_df.columns
        if "cat2" in str(col) and "is_selected" in str(col)
    ]
    assert mahalanobis_cat2_cols, (
        f"No column for 'cat2,is_selected' found in mahalanobis matrix: {list(mahalanobis_df.columns)}"
    )
    mahalanobis_cat2_col = mahalanobis_cat2_cols[0]

    # Check that the mahalanobis columns have values (not all NaN)
    assert not pd.isna(mahalanobis_df[mahalanobis_cat1_col]).all(), (
        f"All values for '{mahalanobis_cat1_col}' in mahalanobis matrix are NaN"
    )
    assert not pd.isna(mahalanobis_df[mahalanobis_cat2_col]).all(), (
        f"All values for '{mahalanobis_cat2_col}' in mahalanobis matrix are NaN"
    )
    ...
    assert isinstance(field_tracking, dict), (
        f"Expected field_tracking to be a dict, but got {type(field_tracking)}"
    )
...
def test_compute_de_with_landmark_handling():
    ...
    # Get the varm keys for group metrics using our helper
    mean_lfc_varm_key, mahalanobis_varm_key = check_group_metrics_varm(
        adata, "de_test_landmarks_with_groups"
    )
    ...
    assert subset1_cols, (
        f"No column for 'subset1' found in mean LFC matrix: {list(mean_lfc_df.columns)}"
    )
    ...
    assert subset2_cols, (
        f"No column for 'subset2' found in mean LFC matrix: {list(mean_lfc_df.columns)}"
    )
    subset2_col = subset2_cols[0]

    # Check that the columns have values (not all NaN)
    assert not pd.isna(mean_lfc_df[subset1_col]).all(), (
        f"All values for '{subset1_col}' in mean LFC matrix are NaN"
    )
    assert not pd.isna(mean_lfc_df[subset2_col]).all(), (
        f"All values for '{subset2_col}' in mean LFC matrix are NaN"
    )

    # Check that mahalanobis columns exist and have values
    mahalanobis_subset1_cols = [
        col for col in mahalanobis_df.columns if "subset1" in str(col)
    ]
    assert mahalanobis_subset1_cols, (
        f"No column for 'subset1' found in mahalanobis matrix: {list(mahalanobis_df.columns)}"
    )
    mahalanobis_subset1_col = mahalanobis_subset1_cols[0]

    mahalanobis_subset2_cols = [
        col for col in mahalanobis_df.columns if "subset2" in str(col)
    ]
    assert mahalanobis_subset2_cols, (
        f"No column for 'subset2' found in mahalanobis matrix: {list(mahalanobis_df.columns)}"
    )
    mahalanobis_subset2_col = mahalanobis_subset2_cols[0]

    # Check that the mahalanobis columns have values (not all NaN)
    assert not pd.isna(mahalanobis_df[mahalanobis_subset1_col]).all(), (
        f"All values for '{mahalanobis_subset1_col}' in mahalanobis matrix are NaN"
    )
    assert not pd.isna(mahalanobis_df[mahalanobis_subset2_col]).all(), (
        f"All values for '{mahalanobis_subset2_col}' in mahalanobis matrix are NaN"
    )
