...
class TestDifferentialExpressionInit:
    ...
    def test_differential_expression_init_basic(self):
        ...
        de = DifferentialExpression(
            n_landmarks=15, eps=1e-6, batch_size=100, max_memory_ratio=0.7
        )
    ...
    def test_differential_expression_init_disk_storage(self):
        ...
        de = DifferentialExpression(
            store_arrays_on_disk=None, disk_storage_dir="/tmp/test"
        )
...
class TestDifferentialExpressionFit:
    ...
    def test_differential_expression_fit_basic(self):
        ...
        X1, X2, expr1, expr2 = create_de_test_data(
            n_cells1=15, n_cells2=20, n_features=4, n_genes=5
        )
    ...
    def test_differential_expression_fit_with_sample_indices(self):
        ...
        de = DifferentialExpression(
            use_sample_variance=None, use_empirical_variance=False
        )

        with (
            patch("kompot.differential.expression_model.mellon") as mock_mellon,
            patch(
                "kompot.differential.expression_model._build_matern52_linear"
            ) as mock_build,
        ):
            mock_estimator = MagicMock()
            mock_predictor = MagicMock()
            ...
            with patch(
                "kompot.differential.expression_model.SampleVarianceEstimator"
            ) as mock_sve:
...
class TestDifferentialExpressionPredict:
    ...
    def test_differential_expression_predict_basic(self):
        ...
        mock_predictor1.return_value = np.array(
            [[1.0, 2.0], [1.5, 2.5], [2.0, 3.0]]
        )  # 3 cells, 2 genes
        mock_predictor2.return_value = np.array([[1.5, 3.0], [2.0, 3.5], [2.5, 4.0]])
        mock_predictor1.covariance.return_value = np.array(
            [[0.1, 0.2], [0.15, 0.25], [0.2, 0.3]]
        )
        mock_predictor2.covariance.return_value = np.array(
            [[0.12, 0.22], [0.17, 0.27], [0.22, 0.32]]
        )
    ...
    def test_differential_expression_predict_with_sample_variance(self):
        ...
        de = DifferentialExpression(
            use_sample_variance=True, use_empirical_variance=False
        )
        ...
        mock_function_predictor1.covariance.return_value = np.array(
            [[0.1, 0.2], [0.15, 0.25]]
        )
        mock_function_predictor2.covariance.return_value = np.array(
            [[0.12, 0.22], [0.17, 0.27]]
        )
        mock_function_predictor1.uncertainty.return_value = np.array(
            [[0.1, 0.2], [0.15, 0.25]]
        )
        mock_function_predictor2.uncertainty.return_value = np.array(
            [[0.12, 0.22], [0.17, 0.27]]
        )
    ...
    def test_differential_expression_predict_custom_thresholds(self):
        ...
        mock_predictor2.return_value = np.array(
            [[3.0, 0.2], [3.0, 0.2]]
        )  # Strong up/down
        ...
        mock_predictor1.uncertainty.return_value = np.array(
            [[0.01, 0.01], [0.01, 0.01]]
        )
        mock_predictor2.uncertainty.return_value = np.array(
            [[0.01, 0.01], [0.01, 0.01]]
        )
...
class TestDifferentialExpressionMemoryManagement:
    ...
    def test_differential_expression_memory_analysis(self):
        ...
        with patch(
            "kompot.memory_utils.analyze_covariance_memory_requirements"
        ) as mock_analyze:
    ...
    def test_differential_expression_disk_storage_setup(self):
        ...
        de = DifferentialExpression(
            store_arrays_on_disk=True, disk_storage_dir="/tmp/test"
        )
    ...
    def test_differential_expression_jax_memory_error_handling(self):
        ...
        with patch("kompot.differential.expression_model.apply_batched") as mock_batch:
            with patch(
                ...
                with patch(
                    "kompot.differential.differential_expression.is_jax_memory_error"
                ) as mock_is_mem_error:
...
class TestDifferentialExpressionBatching:
    ...
    def test_differential_expression_batched_prediction(self):
        ...
        de = DifferentialExpression(
            batch_size=2, use_empirical_variance=False
        )  # Small batch size
