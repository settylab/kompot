...
def test_sample_variance_estimator_diag():
    ...
    for i in range(n_cells):
        np.testing.assert_allclose(
            variance_diag_false[i, i, :], variance_diag_true[i, :], rtol=1e-5, atol=1e-8
        )

    # Verify symmetry of the covariance matrix
    for g in range(n_genes):
        gene_cov = variance_diag_false[:, :, g]
        np.testing.assert_allclose(gene_cov, gene_cov.T, rtol=1e-5, atol=1e-8)
...
def test_sample_variance_estimator_jit():
    ...
    np.testing.assert_allclose(jit_result, no_jit_result, rtol=1e-5, atol=1e-8)
...
def test_memory_sensitive_batch_reduction():
    ...
    with patch("kompot.batch_utils.is_jax_memory_error", return_value=True):
        ...
        result = apply_batched(mock_batch_func, X, batch_size=15)
...
@pytest.mark.skipif(
    ...
    reason="Python 3.9 module resolution issues with mock patching",
)
def test_analyze_memory_called_only_once():
    ...
    with patch.object(
        sample_variance_estimator, "analyze_covariance_memory_requirements"
    ) as mock_analyze:
        ...
        assert (
            not hasattr(estimator, "_memory_analyzed") or not estimator._memory_analyzed
        )
...
def test_disk_backed_sample_variance():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        disk_estimator = SampleVarianceEstimator(
            store_arrays_on_disk=True, disk_storage_dir=temp_dir
        )
...
def test_differential_expression_with_sample_variance_disk_backed():
    ...
    condition1_samples = np.concatenate(
        ...
        + [np.full(n_cells_1 % n_groups, n_groups - 1)]
    ...
    condition2_samples = np.concatenate(
        ...
        + [np.full(n_cells_2 % n_groups, n_groups - 1)]
    )  # Remainder goes to last group
    ...
    de_memory = DifferentialExpression(
        use_sample_variance=True, jit_compile=False, store_arrays_on_disk=False
    )
    ...
    with patch("kompot.batch_utils.is_jax_memory_error", return_value=False):
        ...
        memory_results = de_memory.predict(
            X_pred, compute_mahalanobis=True, progress=False
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert np.all(np.isfinite(memory_results["mahalanobis_distances"])), (
                "Memory-based results contain NaN or Inf"
            )
            assert np.all(np.isfinite(disk_results["mahalanobis_distances"])), (
                "Disk-based results contain NaN or Inf"
            )
...
def test_sample_variance_estimator_invalid_type():
    ...
    with pytest.raises(
        ValueError, match="estimator_type must be either 'function' or 'density'"
    ):
...
def test_sample_variance_estimator_function_mode_without_y():
    ...
    with pytest.raises(
        ValueError, match="Y must be provided for function estimator type"
    ):
        estimator.fit(X=X, grouping_vector=grouping_vector)
