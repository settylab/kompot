...
def create_test_data_with_multiple_runs():
    ...
    _ = compute_differential_abundance(
        adata, groupby="group", condition1="A", condition2="B", result_key="da_run1"
    )
    ...
    adata.uns["da_run2"]["run_info"] = {
        "field_names": {"lfc_key": lfc_key_da, "ptp_key": ptp_key_da}
    }
    ...
    global_history.append(
        {
            ...
            "field_names": {
                ...
                "da": {"lfc_key": lfc_key_da, "ptp_key": ptp_key_da},
            },
        }
    )
...
class TestKeyInferenceFunctions:
    ...
    def test_infer_de_keys_with_run_id(self):
        ...
        assert "de_run2" in lfc_key, (
            f"Expected 'de_run2' in inferred key, got {lfc_key}"
        )
        ...
        if de_keys:
            ...
            assert "de_run1" in lfc_key, (
                f"Expected 'de_run1' in inferred key, got {lfc_key}"
            )
    ...
    def test_infer_da_keys_with_run_id(self):
        ...
        assert "da_run2" in lfc_key, (
            f"Expected 'da_run2' in inferred key, got {lfc_key}"
        )
        ...
        if da_keys:
            lfc_key, ptp_key, thresholds = _infer_da_keys(
                self.adata, lfc_key=da_keys[0]
            )
            assert "da_run1" in lfc_key, (
                f"Expected 'da_run1' in inferred key, got {lfc_key}"
            )
...
class TestPlotFunctions:
    ...
    def test_volcano_de_with_run_id(self):
        ...
        if de_keys:
            fig = volcano_de(
                self.adata, lfc_key=de_keys[0], score_key="test_score", return_fig=True
            )
            assert fig is not None
            assert fig is not None
    ...
    def test_volcano_da_with_run_id(self):
        ...
        fig = volcano_da(self.adata, run_id=-1, return_fig=True)
        assert fig is not None

        # The variant below should also work by directly providing keys
        da_keys_lfc = [
            k for k in self.adata.obs.columns if "da_run1" in k and "lfc" in k
        ]
        da_keys_ptp = [
            k for k in self.adata.obs.columns if "da_run1" in k and "ptp" in k
        ]
    ...
    def test_heatmap_with_run_id(self):
        ...
        pytest.skip(
            "Skipping heatmap test due to matplotlib issues in test environment"
        )
    ...
    def test_gene_expression_plot(self):
        ...
        self.adata.var["de_run2_mahalanobis_A_to_B"] = np.abs(
        ...
        if (
            "kompot_de" in self.adata.uns
            and "run_history" in self.adata.uns["kompot_de"]
        ):
        ...
        fig = plot_gene_expression(
            self.adata, gene=gene, run_id=-1, basis="X_pca", return_fig=True
        )
        assert fig is not None
        assert len(fig.axes) > 0
    ...
    def test_infer_expression_keys(self):
        ...
        lfc_key, score_key = _infer_expression_keys(
            self.adata, lfc_key="kompot_lfc_key", score_key="kompot_score"
        )
        ...
        lfc_key, score_key = _infer_expression_keys(
            self.adata, lfc_key=None, score_key=None
        )
        assert lfc_key is not None
    ...
    def test_direction_barplot(self):
        ...
        categories = np.random.choice(
            ["Type1", "Type2", "Type3"], size=self.adata.n_obs
        )
        ...
        # Test with run_id parameter
        fig = direction_barplot(
            self.adata, category_column="cell_type", run_id=-1, return_fig=True
        )
        assert fig is not None
    ...
    def test_infer_direction_key(self):
        ...
        dir_key, cond1, cond2 = _infer_direction_key(
            self.adata, direction_column="kompot_da_A_to_B_lfc_direction"
        )
    ...
    def test_multi_volcano_da_with_run_id(self):
        ...
        fig = multi_volcano_da(
            self.adata, groupby="cell_type", run_id=-1, return_fig=True
        )
        assert fig is not None
        axes = fig.axes
    ...
    def test_multi_volcano_da_with_highlight_subset(self):
        ...
        highlight_mask = np.random.choice(
            [True, False], size=self.adata.n_obs, p=[0.2, 0.8]
        )
    ...
    def test_multi_volcano_da_with_direction_update(self):
        ...
        if direction_col not in self.adata.obs:
            self.adata.obs[direction_col] = np.random.choice(
                ["up", "down", "neutral"], size=self.adata.n_obs
            )

        # Add direction column to run info for the latest run
        if (
            "kompot_da" in self.adata.uns
            and "run_history" in self.adata.uns["kompot_da"]
        ):
            # Handle the case where run_history is a JSON string
            run_history = self.adata.uns["kompot_da"]["run_history"]
            if isinstance(run_history, str):
            ...
            for run in run_history:
                if (
                    isinstance(run, dict) and run.get("run_id") == 0
                ):  # Latest run in test data
        ...
        assert direction_col in self.adata.obs, (
            f"Direction column {direction_col} not found in adata.obs"
        )
