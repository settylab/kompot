...
class TestUtilsCoverage:
    ...
    def test_find_landmarks_basic(self):
        ...
        assert (
            landmarks.shape[0] <= 25
        )  # Algorithm may find slightly more than requested due to clustering
    ...
    def test_find_landmarks_with_groups(self):
        ...
        assert (
            landmarks.shape[0] <= 25
        )  # May return more/fewer than requested due to clustering algorithm
        assert landmarks.shape[1] == 10
        assert len(landmark_indices) == landmarks.shape[0]
    ...
    def test_find_landmarks_stratified(self):
        ...
        assert (
            landmarks.shape[0] <= 20
        )  # Algorithm may find more than requested due to clustering
        assert landmarks.shape[1] == 10
        assert len(landmark_indices) == landmarks.shape[0]
...
class TestBatchUtilsCoverage:
    ...
    def test_merge_batch_results_arrays(self):
        ...
        batch_results = [np.array([1, 2, 3]), np.array([4, 5]), np.array([6, 7, 8, 9])]
