...
def create_test_anndata_with_underrepresentation(n_cells=60, n_genes=20):
    ...
    obs = pd.DataFrame({"condition": conditions, "tissue": tissues})
    ...
    print(
        f"Distribution: tissue1_A={tissue1_A}, tissue1_B={tissue1_B}, tissue2_A={tissue2_A}, tissue2_B={tissue2_B}"
    )
...
def test_check_underrepresentation_basic():
    ...
    assert "__underrepresentation_data" in result, (
        f"__underrepresentation_data should be in result: {result}"
    )
    ...
    assert "tissue1" in underrep_data, (
        f"tissue1 should be in underrep_data: {underrep_data}"
    )
    assert "tissue2" in underrep_data, (
        f"tissue2 should be in underrep_data: {underrep_data}"
    )
    assert "B" in underrep_data["tissue1"], (
        f"B should be underrepresented in tissue1: {underrep_data['tissue1']}"
    )
    assert "A" in underrep_data["tissue2"], (
        f"A should be underrepresented in tissue2: {underrep_data['tissue2']}"
    )
    ...
    assert "tissue1" in tissues and "tissue2" in tissues, (
        f"Expected tissues in list: {tissues}"
    )
    ...
    assert isinstance(result_with_min_cells, dict), (
        f"Expected a dictionary, got: {type(result_with_min_cells)}"
    )

    # We should have both __underrepresentation_data and tissue keys
    assert "__underrepresentation_data" in result_with_min_cells, (
        f"Expected __underrepresentation_data in result, got: {result_with_min_cells}"
    )
    assert "tissue" in result_with_min_cells, (
        f"Expected tissue in result, got: {result_with_min_cells}"
    )

    # Check detailed underrepresentation data
    underrep_data = result_with_min_cells["__underrepresentation_data"]
    assert "tissue2" in underrep_data, (
        f"Expected tissue2 in underrep_data, got: {underrep_data}"
    )
    assert "A" in underrep_data["tissue2"], (
        f"Expected A in tissue2, got: {underrep_data['tissue2']}"
    )

    # tissue1/B might not be here as it has 3 cells which equals min_cells
    if "tissue1" in underrep_data:
        assert "B" in underrep_data["tissue1"], (
            f"Expected B in tissue1, got: {underrep_data['tissue1']}"
        )
    ...
    assert isinstance(tissues, list), (
        f"Expected tissues to be a list, got: {type(tissues)}"
    )
...
def test_check_underrepresentation_with_different_group_types():
    ...
    filter_key = [k for k in result_array.keys() if k != "__underrepresentation_data"][
        0
    ]
...
def test_compute_de_with_check_representation_none():
    ...
    with patch("logging.Logger.warning") as mock_warning:
        with patch("logging.Logger.info") as mock_info:
            ...
            assert mock_warning.called, (
                "Warning should be logged with check_representation=None"
            )
            ...
            assert not auto_filter_calls, (
                "No auto-filtering should occur with check_representation=None"
            )
    ...
    assert run_info["underrepresentation"] is not None, (
        "underrepresentation should not be None"
    )
    assert len(run_info["underrepresentation"]) > 0, (
        "underrepresentation dict should not be empty"
    )

    # Check that tissue2/A is underrepresented (consistently found in our test data)
    assert "tissue2" in run_info["underrepresentation"], (
        f"Expected tissue2 in underrepresentation: {run_info['underrepresentation']}"
    )
    assert "A" in run_info["underrepresentation"]["tissue2"], (
        f"Expected A to be underrepresented in tissue2: {run_info['underrepresentation']['tissue2']}"
    )
...
def test_compute_de_with_check_representation_true():
    ...
    with patch("logging.Logger.info") as mock_info:
        ...
        assert underrep_check_calls, (
            "Should log checking for underrepresentation with check_representation=True"
        )
...
def test_compute_de_with_check_representation_true_and_filter():
    ...
    with patch("kompot.anndata._de_helpers.apply_cell_filter") as mock_apply:
        ...
        if mock_apply.called:
            ...
            assert "cell_filter" in kwargs, (
                "cell_filter should be passed to apply_cell_filter"
            )
            assert "groups" in kwargs, "groups should be passed to apply_cell_filter"
            assert "check_representation" in kwargs, (
                "check_representation should be passed to apply_cell_filter"
            )
            assert "groupby" in kwargs, "groupby should be passed to apply_cell_filter"
            assert "conditions" in kwargs, (
                "conditions should be passed to apply_cell_filter"
            )
            ...
            assert kwargs["check_representation"] == True, (
                "check_representation should be True"
            )
            assert set(kwargs["conditions"]) == set(["A", "B"]), (
                "conditions should be ['A', 'B']"
            )
            assert kwargs["cell_filter"] == initial_filter, (
                "cell_filter should match initial_filter"
            )
    ...
    with patch("logging.Logger.warning") as mock_warning:
        with patch("logging.Logger.info") as mock_info:
            ...
            assert not check_calls, (
                "No checking messages should be logged with check_representation=False"
            )
...
def test_refine_filter_for_underrepresentation():
    ...
    tissue2_A_count = np.sum(
        (filtered_adata.obs["tissue"] == "tissue2")
        & (filtered_adata.obs["condition"] == "A")
    )
    assert tissue2_A_count < 10, (
        f"Should still have underrepresented condition A in tissue2: {tissue2_A_count}"
    )
    ...
    assert np.sum(refined_mask) < np.sum(filter_mask), (
        "Refined mask should exclude more cells"
    )
    ...
    if np.any(tissue2_mask_refined):
        tissue2_A_count_refined = np.sum(
            (refined_adata.obs["tissue"] == "tissue2")
