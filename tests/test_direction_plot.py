...
def create_test_anndata(n_cells=60, n_genes=20):
    ...
    obs = pd.DataFrame({"group": groups, "cell_type": cell_types})
...
def test_infer_direction_key_multiple_columns():
    ...
    if "kompot_da" in adata.uns and "run_history" in adata.uns["kompot_da"]:
        ...
        from kompot.anndata.utils import get_run_history
    ...
    direction_column, condition1, condition2 = _infer_direction_key(adata, run_id=-1)

    # Check results
    assert direction_column is not None
    assert "direction" in direction_column
    assert "da_run2" in direction_column, (
        "Should find the most recent run's direction column"
    )
    assert condition1 == "A"
    assert condition2 == "B"
...
def test_direction_barplot_normalization_options():
    ...
    fig = direction_barplot(
        adata, category_column="cell_type", run_id=-1, normalize=False, return_fig=True
    )
...
def test_direction_barplot_stacked_vs_grouped():
    ...
    fig = direction_barplot(
        adata, category_column="cell_type", run_id=-1, stacked=True, return_fig=True
    )
    ...
    fig = direction_barplot(
        adata, category_column="cell_type", run_id=-1, stacked=False, return_fig=True
    )
...
def test_direction_barplot_custom_colors():
    ...
    custom_colors = {"up": "darkred", "down": "navy", "neutral": "lightgray"}
