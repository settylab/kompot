...
def create_test_adata(n_cells=200, n_genes=50):
    ...
    obs = pd.DataFrame(
        {
            "cell_type": pd.Categorical(
                np.random.choice(["TypeA", "TypeB", "TypeC"], n_cells)
            ),
            "condition": pd.Categorical(
                np.random.choice(["control", "treatment"], n_cells)
            ),
    ...
    obsm = {"X_pca": np.random.normal(0, 1, (n_cells, 10))}
...
def test_parse_groups_formatted_names():
    ...
    subset_masks, subset_names = parse_groups(
        adata, "cell_with_spaces", formatted_names=True
    )
    ...
    # Test with dashes
    subset_masks, subset_names = parse_groups(
        adata, "cell_with_dashes", formatted_names=True
    )
    for name in subset_names:
        assert (
            "-" not in name or name == "Type-A" or name == "Type-B"
        )  # Original may remain
...
def test_parse_groups_return_description():
    ...
    subset_masks, description = parse_groups(
        adata, "cell_type", return_description=True
    )
...
def test_parse_groups_dict_with_list_values():
    ...
    groups_dict = {"cell_type": ["TypeA", "TypeB"], "condition": "control"}
...
def test_parse_groups_nested_dict_with_list():
    ...
    expected_group2 = (
        (adata.obs["condition"] == "treatment") & (adata.obs["cell_type"] == "TypeC")
    ).values
...
def test_apply_cell_filter_dict():
    ...
    # Dictionary filter
    cell_filter = {"cell_type": "TypeA", "condition": "control"}
    ...
    expected = (
        (adata.obs["cell_type"] == "TypeA") & (adata.obs["condition"] == "control")
    ).values
...
def test_apply_cell_filter_list_of_dicts():
    ...
    cell_filter = [{"cell_type": "TypeA"}, {"cell_type": "TypeB"}]
    ...
    expected = (
        (adata.obs["cell_type"] == "TypeA") | (adata.obs["cell_type"] == "TypeB")
    ).values
    np.testing.assert_array_equal(mask, expected)
...
def test_apply_cell_filter_list_values():
    ...
    cell_filter = {"cell_type": ["TypeA", "TypeB"]}
...
def test_check_underrepresentation_edge_cases():
    ...
    small_groups = {"small": np.array([True, False, True] + [False] * 47)}
