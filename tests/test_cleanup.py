...
class TestCleanupFunctionality:
    ...
    def test_cleanup_remove_var_fields(self):
        ...
        # Remove all fields
        cleanup(adata, keep_layers=False, keep_var_fields=False)
    ...
    def test_get_field_status(self):
        ...
        for location, types in status_before.items():
            for field_type, fields in types.items():
                for field_name, is_present in fields.items():
                    assert is_present, (
                        f"Field {field_name} should be present before cleanup"
                    )
            ...
            for field_type, fields in status_after["layers"].items():
                for field_name, is_present in fields.items():
                    assert not is_present, (
                        f"Layer field {field_name} should be missing after cleanup"
                    )
        ...
        if "var" in status_after:
            for field_type, fields in status_after["var"].items():
                for field_name, is_present in fields.items():
                    assert is_present, (
                        f"Var field {field_name} should still be present after cleanup"
                    )
...
class TestRunInfoMissingFields:
    ...
    def test_runinfo_detects_missing_fields(self):
        ...
        assert len(run_info_before.missing_fields) == 0, (
            "No fields should be missing before cleanup"
        )
        ...
        assert len(run_info_after.missing_fields) > 0, (
            "Some fields should be missing after cleanup"
        )
