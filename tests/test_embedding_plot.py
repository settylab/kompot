...
pytestmark = pytest.mark.skipif(
    not _has_scanpy, reason="Scanpy is required for these tests"
)
...
def test_embedding_with_groups():
    ...
    with patch("matplotlib.pyplot.show"):  # Mock plt.show to prevent display
        fig = embedding(
            adata, basis="umap", color="cluster", groups=groups, return_fig=True
        )
...
def test_embedding_colormap_parameters():
    ...
    with patch("matplotlib.pyplot.show"):
        fig = embedding(
            adata, basis="umap", color="score", color_map="viridis", return_fig=True
        )
    ...
    with patch("matplotlib.pyplot.show"):
        fig = embedding(
            adata, basis="umap", color="score", cmap="plasma", return_fig=True
        )
...
def test_embedding_with_ax():
    ...
    result = embedding(adata, basis="umap", color="cluster", ax=ax, return_fig=False)
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    result = embedding(adata, basis="umap", color="cluster", ax=ax, return_fig=True)
...
def test_embedding_with_mgroups():
    ...
    with patch("matplotlib.pyplot.show"):
        result = embedding(
            adata, basis="umap", color="cluster", mgroups=mgroups, return_fig=True
        )
...
def test_embedding_mgroups_with_color_list_error():
    ...
    # Define multiple group conditions
    mgroups = [{"condition": "control"}, {"condition": "treatment"}]
