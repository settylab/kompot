...
def create_large_test_data(
    n_points: int = 1000, n_genes: int = 100, seed: int = 42
) -> Dict[str, np.ndarray]:
...
class MemoryMonitor:
    ...
    def __enter__(self):
        ...
        logger.info(
            f"Starting {self.operation_name} - Initial memory: {self.start_memory:.2f} MB"
        )
        return self
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        logger.info(
            f"Finished {self.operation_name} - Final memory: {self.end_memory:.2f} MB"
        )
        logger.info(
            f"Memory usage for {self.operation_name}: Start={self.start_memory:.2f}MB, "
...
@pytest.mark.skipif(
    not PSUTIL_AVAILABLE, reason="psutil not available for memory monitoring"
)
class TestMemoryValidation:
    ...
    def test_memory_usage_comparison_small_dataset(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with MemoryMonitor(
                "Disk-backed sample variance computation"
            ) as disk_monitor:
        ...
        assert correlation > 0.5, (
            f"Results should be reasonably correlated, got {correlation:.3f}"
        )
        ...
        if memory_usage_disk_backed > memory_usage_in_memory + max_acceptable_overhead:
            pytest.fail(
                f"Disk-backed version used too much more memory: "
    ...
    def test_memory_usage_large_dataset_simulation(self):
        ...
        logger.info(
            f"Testing with simulated large dataset: {n_cells} cells, {n_genes} genes"
        )
        ...
        with MemoryMonitor("In-memory large dataset simulation") as memory_monitor:
            estimator_memory = SampleVarianceEstimator(
                estimator_type="function", store_arrays_on_disk=False
            )

            # Set up mock predictors
            estimator_memory.group_predictors = {
                f"group_{i}": MockPredictor(i * 0.5, seed=42 + i)
        ...
        logger.info("Large dataset memory usage comparison:")
        logger.info(f"  In-memory: {memory_usage_in_memory:.2f} MB")
        logger.info(f"  Disk-backed: {memory_usage_disk_backed:.2f} MB")

        # Validate that both approaches produce results of correct shape
        expected_shape = (n_cells, n_cells, n_genes)
        assert result_memory.shape == expected_shape, (
            f"Memory result shape: {result_memory.shape}"
        )
        assert result_disk.shape == expected_shape, (
            f"Disk result shape: {result_disk.shape}"
        )
        ...
        memory_savings_percent = (
            (memory_savings / memory_usage_in_memory) * 100
            if memory_usage_in_memory > 0
            else 0
        )

        logger.info(
            f"Memory savings: {memory_savings:.2f} MB ({memory_savings_percent:.1f}%)"
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if DASK_AVAILABLE:
                assert isinstance(result, da.Array), (
                    "Result should be a Dask array when dask is available"
                )
            else:
                assert isinstance(result, np.memmap), (
                    "Result should be a memory-mapped array when dask is not available"
                )
            ...
            if isinstance(result, np.memmap):
                assert hasattr(result, "filename"), (
                    "Memory-mapped array should have filename attribute"
                )
            ...
            logger.info("Disk-backed array validation passed:")
            ...
            if isinstance(result, np.memmap):
                ...
                result[0, 0, 0] = 999.0
                assert result[0, 0, 0] == 999.0, (
                    "Should be able to modify memory-mapped array"
                )
                result[0, 0, 0] = original_value  # Restore
                logger.info(f"  File: {result.filename}")
                logger.info(
                    f"  Size on disk: {os.path.getsize(result.filename) / 1024 / 1024:.2f} MB"
                )
    ...
    def test_garbage_collection_effectiveness(self):
        ...
        try:
            ...
            logger.info(
                f"After allocating numpy arrays: {after_allocation:.2f} MB (+{memory_increase:.1f} MB)"
            )
