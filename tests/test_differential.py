...
def test_differential_abundance_sync_parameters():
    ...
    correlation_density1 = np.corrcoef(
        pred_nosync["log_density_condition1"], pred_sync["log_density_condition1"]
    )[0, 1]
    correlation_density2 = np.corrcoef(
        pred_nosync["log_density_condition2"], pred_sync["log_density_condition2"]
    )[0, 1]
...
def test_differential_abundance_predict_with_thresholds():
    ...
    # Fit the model
    diff_abundance = DifferentialAbundance(
        log_fold_change_threshold=1.0, ptp_threshold=0.05
    )
    diff_abundance.fit(X_condition1, X_condition2)
    ...
    np.testing.assert_array_equal(
        default_predictions["log_fold_change"], strict_predictions["log_fold_change"]
    )
    np.testing.assert_array_equal(
        default_predictions["log_fold_change"], loose_predictions["log_fold_change"]
    )
    ...
    default_significant = np.sum(
        default_predictions["log_fold_change_direction"] != "neutral"
    )
    strict_significant = np.sum(
        strict_predictions["log_fold_change_direction"] != "neutral"
    )
    loose_significant = np.sum(
        loose_predictions["log_fold_change_direction"] != "neutral"
    )
...
def test_differential_abundance_with_sample_variance():
    ...
    condition1_sample_indices = np.array(
        [0] * n_cells_per_sample + [1] * n_cells_per_sample + [2] * n_cells_per_sample
    )
    condition2_sample_indices = np.array(
        [0] * n_cells_per_sample + [1] * n_cells_per_sample + [2] * n_cells_per_sample
    )
    ...
    diff_abundance_no_variance = DifferentialAbundance(use_sample_variance=False)
    diff_abundance_no_variance.fit(X_condition1, X_condition2)

    # Create and fit model with sample variance
    diff_abundance_with_variance = DifferentialAbundance(use_sample_variance=True)
    ...
    assert np.mean(pred_with_variance["log_fold_change_uncertainty"]) >= np.mean(
        pred_no_variance["log_fold_change_uncertainty"]
    )
    ...
    assert np.mean(np.abs(pred_with_variance["log_fold_change_zscore"])) <= np.mean(
        np.abs(pred_no_variance["log_fold_change_zscore"])
    )
    ...
    n_landmarks = 20
    X_combined = np.vstack([X_condition1, X_condition2])
    landmarks = X_combined[
        np.random.choice(len(X_combined), n_landmarks, replace=False)
    ]

    diff_abundance_with_landmarks = DifferentialAbundance(use_sample_variance=True)
...
def test_differential_expression_fit():
    ...
    X_combined = np.vstack([X_condition1, X_condition2])
    predictions = diff_expression.predict(
        X_combined, compute_mahalanobis=False
    ...
    assert np.allclose(
        predictions["mean_log_fold_change"], another_predictions["mean_log_fold_change"]
    )
...
def test_differential_expression_predict():
    ...
    diff_expression_for_predictors.fit(
    ...
    diff_expression_with_function_predictors.fit(
        X_condition1, y_condition1, X_condition2, y_condition2
    )

    # Predict using precomputed function predictors
    predictions_with_function_predictors = (
        diff_expression_with_function_predictors.predict(X_new)
    )
...
def test_update_direction_column():
        ...
        obs=pd.DataFrame(
            {
                ...
                "neg_log10_ptp": -np.log10(
                    np.random.random(n_cells) * 0.1
                ),  # Same but transformed
                "original_direction": np.random.choice(
                    ["up", "down", "neutral"], n_cells
                ),
            }
        ),
    )
    ...
    # Should have all three direction categories in correct order
    assert list(adata_copy.obs["new_direction"].cat.categories) == [
        "up",
        "neutral",
        "down",
    ]
