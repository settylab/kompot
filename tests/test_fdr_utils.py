...
class TestComputeFDRStatistics:
    ...
    def test_local_fdr_monotone_decreasing(self):
        ...
        _, local_fdr_vals, _, _ = compute_fdr_statistics(real_mahalanobis, null_mahalanobis, 0.05)
    ...
    def test_densities_strictly_declining(self):
        ...
        assert abs(integral - 1.0) < 0.05, f"Density integral = {integral}, expected ~1.0"
...
class TestGroundTruth:
    ...
    def test_pure_null_no_discoveries(self):
        ...
        _, est_lfdr, _, is_sig = compute_fdr_statistics(real_distances, null_distances, 0.05)
        ...
        assert np.median(est_lfdr) > 0.8, f"Median lfdr = {np.median(est_lfdr):.3f}, expected > 0.8"
    ...
    def test_strong_separation_high_sensitivity(self):
        ...
        real_distances = np.concatenate([real_null, real_alt])
        is_truly_de = np.concatenate([np.zeros(len(real_null)), np.ones(len(real_alt))]).astype(bool)
    ...
    def test_varying_signal_strength(self):
        ...
        for lambda1 in [1.0, 0.5, 0.2]:
            ...
            _, _, _, is_sig = compute_fdr_statistics(real_distances, null_distances, 0.05)
    ...
    def test_fdr_control_at_threshold(self):
        ...
        _, est_lfdr, _, _ = compute_fdr_statistics(real_distances, null_distances, threshold)
    ...
    def test_concordance_with_grenander_ecdf(self):
        ...
        assert np.all(np.diff(f_gren) <= 1e-10), "Grenander mixture density not declining"
    ...
    def test_gamma_distributed_ground_truth(self):
        ...
        sensitivity = np.sum(is_sig & is_truly_de) / np.sum(is_truly_de)
        assert sensitivity > 0.3, f"Sensitivity too low for gamma ground truth: {sensitivity:.3f}"
