...
def create_test_anndata_with_fdr(n_cells=100, n_genes=500, n_differential=50, 
    ...
    lfc[diff_indices] = np.random.choice([-3, 3], n_differential) + np.random.normal(0, 0.5, n_differential)
    ...
    local_fdr[diff_indices] = np.random.beta(1, 20, n_differential)  # Lower values (more significant)
...
class TestVolcanoFDRBasicFunctionality:
    ...
    def test_fdr_transformation(self):
        ...
        assert len(all_y_data) == len(expected_y), f"Expected {len(expected_y)} total points, got {len(all_y_data)}"
...
class TestVolcanoFDRColoring:
    ...
    def test_manual_background_coloring_override(self):
        ...
        assert len(custom_categories) == 3, f"Should have 3 custom categories, got {len(custom_categories)}"
    ...
    def test_de_column_detection(self):
        ...
        assert final_count == initial_count, "DE column should be unchanged without update_de_classification"
        plt.close(fig)
...
class TestVolcanoFDRThresholds:
    ...
    def test_threshold_position_correct(self):
        ...
        horizontal_lines = [line for line in lines if 
                          len(set(line.get_ydata())) == 1 and  # horizontal line
                          line.get_ydata()[0] > 0]  # above x-axis
...
class TestVolcanoFDRClassificationUpdates:
    ...
    def test_update_classification_enabled(self):
        ...
        final_count = np.sum(adata.var['kompot_de_is_de'])
        assert final_count >= initial_count, "Should have at least as many significant genes with lenient threshold"
        plt.close(fig)
    ...
    def test_update_classification_strict_threshold(self):
        ...
        final_count = np.sum(adata.var['kompot_de_is_de'])
        assert final_count <= initial_count, "Should have fewer or equal significant genes with strict threshold"
        plt.close(fig)
...
class TestVolcanoFDRErrorHandling:
    ...
    def test_missing_fdr_columns(self):
        ...
        adata.var = adata.var.drop(columns=[
            'kompot_de_mahalanobis_local_fdr',
            'kompot_de_mahalanobis_tail_fdr'
        ])
    ...
    def test_group_specific_fdr_warning(self):
        ...
        adata.varm['kompot_de_mean_lfc'] = pd.DataFrame({
            'group1': np.random.normal(0, 1, len(adata.var))
        }, index=adata.var_names)
        adata.varm['kompot_de_mahalanobis'] = pd.DataFrame({
            'group1': np.random.gamma(2, 1, len(adata.var))
        }, index=adata.var_names)
