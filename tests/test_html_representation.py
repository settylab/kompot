...
from kompot.anndata import compute_differential_abundance, RunInfo
...
def test_runinfo_html_representation():
    ...
    compute_differential_abundance(
        adata, groupby="group", condition1="A", condition2="B", result_key="test_html"
    )
...
def test_shared_fields_implementation():
    ...
    html.append(
        "<tr><td colspan='4' style='background-color:#e3f2fd; font-weight:bold;'>OBS Shared Fields</td></tr>"
    )

    # This is the explanation text that should appear
    if has_overlapping_fields:
        html.append(
            "<div style='margin-top: 10px; font-size: 0.9em; background-color:#f9f9f9; padding:8px; border-radius:4px;'>"
        )
        html.append(
