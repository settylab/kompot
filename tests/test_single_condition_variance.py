...
class TestSingleConditionVariance:
    ...
    def setup_method(self):
        ...
        samples = (
            ["sample1"] * 15 + ["sample2"] * 15 + ["sample3"] * 30
        )  # cond2 has only one sample
    ...
    def test_both_conditions_multiple_samples_works(self):
        ...
        samples_both = (
            ["sample1"] * 15 + ["sample2"] * 15 + ["sample3"] * 15 + ["sample4"] * 15
        )
    ...
    def test_single_variance_fallback_mechanism(self):
        ...
        samples_reverse = (
            ["sample1"] * 30 + ["sample2"] * 15 + ["sample3"] * 15
        )  # cond1 has only one sample
    ...
    def test_both_conditions_fail_raises_error(self):
        ...
        samples_both_fail = ["sample1"] * 30 + [
            "sample2"
        ] * 30  # Each condition has only one sample
