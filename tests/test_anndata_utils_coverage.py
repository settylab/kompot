    ...
    obs = pd.DataFrame(
        {
            ...
            "sample": [
                f"sample_{i // 5}" for i in range(n_cells)
            ],  # 5 cells per sample
        }
    )
...
class TestGroupUtilsCoverage:
    ...
    def test_parse_groups_formatted_names(self, utils_adata):
        ...
        groups_dict, group_names = parse_groups(
            utils_adata, "group", formatted_names=True
        )
        assert len(group_names) == 2  # Two groups: A and B
...
class TestFieldTrackingCoverage:
    ...
    def test_generate_output_field_names_da(self):
        ...
        field_names = generate_output_field_names(
            result_key="test_da", condition1="A", condition2="B", analysis_type="da"
        )
    ...
    def test_generate_output_field_names_with_sample_suffix(self):
        ...
        assert any(
            "sample_var" in str(v) for v in field_names.values() if isinstance(v, str)
        )
    ...
    def test_detect_output_field_overwrite(self, utils_adata):
        ...
        field_names = {"lfc_key": "new_field", "existing_key": "existing_field"}

        result = detect_output_field_overwrite(
            utils_adata, field_names=field_names, analysis_type="de"
        )
        assert isinstance(result, tuple)
        assert len(result) == 3

        # Test with result_type instead
        result2 = detect_output_field_overwrite(
            utils_adata, field_names=field_names, result_type="differential_expression"
        )
    ...
    def test_validate_field_run_id(self, utils_adata):
        ...
        field_tracking = {"obs": {"test_field": 1}, "var": {}, "uns": {}}
    ...
    def test_validate_field_run_id_mismatch(self, utils_adata):
        ...
        # Set up field tracking data
        field_tracking = {"obs": {"test_field": 2}, "var": {}, "uns": {}}
        set_json_metadata(utils_adata, "kompot_da.anndata_fields", field_tracking)
