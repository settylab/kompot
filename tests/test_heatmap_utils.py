...
from kompot.plot.heatmap.visualization import _draw_fold_change_cell

# Reuse the test data generation functions
from tests.test_plot_functions import (
    create_test_anndata,
    create_test_data_with_multiple_runs,
)
...
class TestHeatmapUtilityFunctions:
    ...
    def setup_class(cls):
        ...
        if cls.test_data is None:
            cls.test_data = (
                create_test_anndata()
            )  # Use simpler creation for utility tests
    ...
    def setup_method(self):
        ...
        self.expr_df = pd.DataFrame(
            np.random.randn(50, 10), columns=[f"gene_{i}" for i in range(10)]
        )
    ...
    def test_prepare_gene_list(self):
        ...
        var_names, score_key, run_info = _prepare_gene_list(
            self.adata, score_key="test_score", n_top_genes=5
        )
        ...
        if "kompot_run_history" in self.adata.uns:
            var_names, score_key, run_info = _prepare_gene_list(
                self.adata, run_id=-1, score_key="test_score", n_top_genes=5
            )
            assert len(var_names) == 5
            assert var_names is not None
            assert score_key == "test_score"
    ...
    def test_get_expression_matrix(self):
        ...
        expr_matrix = _get_expression_matrix(
            self.adata, var_names, layer="non_existent"
        )
        assert expr_matrix is not None
        assert expr_matrix.shape[1] == len(var_names)
        assert expr_matrix.shape[0] == self.adata.n_obs
    ...
    def test_filter_excluded_groups(self):
        ...
        test_df = pd.DataFrame(
            {"gene": np.random.randn(50), "group": ["A"] * 25 + ["B"] * 25}
        )

        # Test with a single excluded group
        filtered_df = _filter_excluded_groups(test_df.copy(), "group", "A", ["A", "B"])
        ...
        with pytest.raises(ValueError):
            _filter_excluded_groups(test_df.copy(), "group", ["A", "B"], ["A", "B"])

        # Test with a non-existent group
        filtered_df = _filter_excluded_groups(test_df.copy(), "group", "C", ["A", "B"])
    ...
    def test_apply_scaling(self):
        ...
        test_df = pd.DataFrame(
            np.random.randn(10, 5), columns=[f"gene_{i}" for i in range(5)]
        )
    ...
    def test_calculate_figsize(self):
        ...
        figsize = _calculate_figsize(
            10, 20, dendrogram=True, cluster_rows=True, cluster_cols=True
        )
...
class TestHeatmapVisualization:
    ...
    def test_draw_fold_change_cell(self):
        ...
        _draw_fold_change_cell(
            ax=ax, x=0, y=0, w=1, h=1, lfc=1.0, cmap="RdBu_r", vmin=-2, vmax=2
        )

        # Test with NaN values
        _draw_fold_change_cell(
            ax=ax, x=1, y=0, w=1, h=1, lfc=np.nan, cmap="RdBu_r", vmin=-2, vmax=2
        )
...
class TestHeatmapWithImplicitRunId:
    ...
    def setup_class(cls):
        ...
        if cls.test_data is None:
            ...
            cls.test_data.obs["condition"] = ["A"] * (cls.test_data.n_obs // 2) + [
                "B"
            ] * (cls.test_data.n_obs // 2)
    ...
    def test_heatmap_with_implicit_run_id(self):
        ...
        if "condition" not in self.adata.obs.columns:
            self.adata.obs["condition"] = ["A"] * (self.adata.n_obs // 2) + ["B"] * (
                self.adata.n_obs // 2
            )
