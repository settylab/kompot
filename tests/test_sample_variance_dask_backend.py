...
def test_disk_storage_initialization():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Test without Dask
        storage_without_dask = DiskStorage(
            temp_dir, namespace="test_no_dask", use_dask=False
        )
...
def test_sample_variance_estimator_disk_storage():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for use_dask in [True, False]:
            ...
            with patch("kompot.memory_utils.DASK_AVAILABLE", use_dask):
                ...
                if (
                    hasattr(estimator, "disk_storage")
                    and estimator.disk_storage is not None
                ):
                    storage_size = estimator.disk_storage.get_directory_size()
                    logger.info(
                        f"Storage size with use_dask={use_dask}: {storage_size} bytes"
                    )
    ...
    logger.info(
        f"Diagonal variance diffs - Max: {max_diag_diff}, Mean: {mean_diag_diff}"
    )
...
if __name__ == "__main__":
    ...
    print("Diagonal variance differences:")
    print(f"  - Max: {results['diag_diffs']['max']}")
    print(f"  - Mean: {results['diag_diffs']['mean']}")

    print("Covariance matrix differences:")
