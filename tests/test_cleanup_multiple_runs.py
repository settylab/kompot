...
class TestCleanupMultipleRuns:
    ...
    def test_cleanup_with_overwritten_fields(self):
        ...
        assert len(run_info_0.overwritten_fields) > 0, (
            "First run should have overwritten fields"
        )
        ...
        assert len(run_info_2.missing_fields) > 0, (
            "Latest run should have missing fields after cleanup"
        )
        ...
        assert len(run_info_0_after.overwritten_fields) > 0, (
            "First run should still show overwritten"
        )
    ...
    def test_missing_takes_precedence_over_overwritten(self):
        ...
        assert len(run_info_0_after.missing_fields) > 0, (
            "First run should show missing fields"
        )
    ...
    def test_cleanup_all_runs(self):
        ...
        for i in range(3):
            ...
            assert len(run_info.missing_fields) > 0, (
    ...
    def test_runinfo_html_display_with_missing_and_overwritten(self):
        ...
        assert "Missing/Deleted" in html or "missing" in html.lower(), (
            "HTML should mention missing fields"
        )
    ...
    def test_cleanup_preserves_run_history(self):
        ...
        assert len(after_history) == original_history_len, (
            "Run history length should not change"
        )
    ...
    def test_get_field_status_multiple_runs(self):
        ...
        if "layers" in status_before:
            for field_type, fields in status_before["layers"].items():
                for field_name, is_present in fields.items():
                    assert is_present, (
                        f"Layer field {field_name} should be present before cleanup"
                    )
        ...
        if "var" in status_before:
            for field_type, fields in status_before["var"].items():
                ...
                for field_name, is_present in fields.items():
                    assert is_present, (
                        f"Var field {field_name} should be present before cleanup"
                    )
        ...
        if "layers" in status_after:
            assert any(
                not is_present
                for fields in status_after["layers"].values()
        ...
        if "layers" in status_run2:
            for field_type, fields in status_run2["layers"].items():
                for field_name, is_present in fields.items():
                    assert is_present, (
                        f"Run 2 layer field {field_name} should still be present"
                    )
        ...
        if "var" in status_run2:
            for field_type, fields in status_run2["var"].items():
                ...
                for field_name, is_present in fields.items():
                    assert is_present, (
                        f"Run 2 var field {field_name} should still be present"
                    )
