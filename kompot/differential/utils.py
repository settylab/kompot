...
def compute_weighted_mean_fold_change(
    ...
    if log_density_diff is None:
        if log_density_condition1 is None or log_density_condition2 is None:
            raise ValueError(
                "Either log_density_diff or both log_density_condition1 and log_density_condition2 must be provided"
            )
...
def update_direction_column(
    ...
    if lfc_threshold is None and auto_lfc_threshold is not None:
        ...
        raise ValueError(
            "No log fold change threshold found. Please provide lfc_threshold."
        )
    ...
    if ptp_threshold is None and auto_ptp_threshold is not None:
        ...
        raise ValueError(
            "No Posterior Tail Probability threshold found. Please provide ptp_threshold."
        )
    ...
    if direction_column is None:
        ...
        if direction_column is None:
            ...
            if direction_column is None:
                raise ValueError(
                    "Could not find direction column. Please provide direction_column."
                )

    logger.info(
        f"Updating direction column '{direction_column}' with thresholds: "
    ...
    adata.obs[direction_column] = pd.Categorical(
        adata.obs[direction_column], categories=["up", "neutral", "down"]
    )
    ...
    logger.info(
        f"Updated direction counts: up={up_count}, down={down_count}, neutral={neutral_count}"
    )
