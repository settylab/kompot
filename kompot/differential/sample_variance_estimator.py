...
class SampleVarianceEstimator:
    ...
    def fit(
        ...
        logger.info(
            f"Found {potential_n_groups:,} unique groups for variance estimation"
        )
        ...
        for group_id, indices in group_indices.items():
            if len(indices) >= min_cells:
                ...
                logger.warning(
                    f"Skipping group {group_id} (only {len(indices):,} cells < min_cells={min_cells:,})"
                )
        ...
        logger.info(
            f"{self.n_groups} groups have sufficient cells (>= {min_cells}) for variance estimation"
        )
        ...
        for group_id, indices in valid_group_indices.items():
            logger.info(
                f"Training estimator for group {group_id} with {len(indices):,} cells"
            )
            X_subset = X[indices]
    ...
    def predict(
        self, X_new: np.ndarray, diag: bool = False, progress: bool = True
    ) -> np.ndarray:
        """
        ...
        if not self.group_predictors:
            error_msg = (
                "No group predictors available. Sample variance estimation failed."
            )
            logger.error(error_msg)
        ...
        if diag:
            ...
            all_group_predictions_jax = [
                jnp.array(pred) for pred in all_group_predictions
            ]

            # Use the JIT-compiled function if available
            if self.jit_compile and self._predict_variance_jit is not None:
                batch_variance = self._predict_variance_jit(
                    X_new, all_group_predictions_jax
                )
            ...
            if use_disk_storage and self._disk_storage is None:
                ...
                self._disk_storage = DiskStorage(
                    storage_dir=self.disk_storage_dir, n_cells=n_cells, n_genes=n_genes
                )
                logger.debug(
                    f"Initializing disk storage for covariance matrix at {self._disk_storage.storage_dir}"
                )
            ...
            if use_disk_storage:
                logger.info(
                    f"Using gene-by-gene disk storage for covariance matrix (shape={covariance_shape})"
                )
            else:
        ...
        means = jnp.mean(
            stacked_predictions, axis=0, keepdims=True
        )  # (1, n_cells, n_genes)
        ...
        if use_disk_storage:
            ...
            if DASK_AVAILABLE:
                ...
                for g in range(n_genes):
                    ...
                    gene_array = da.from_delayed(
                        delayed_result, shape=(n_cells, n_cells), dtype=np.float64
                    )
                ...
                logger.info(
                    f"Created Dask array for covariance with shape {dask_covariance.shape}"
                )
                ...
                logger.info(
                    "Using memory-mapped arrays for disk-backed covariance computation"
                )
                ...
                filename = os.path.join(
                    self._disk_storage.storage_dir, "covariance_matrix.npy"
                )
                mmap_array = np.lib.format.open_memmap(
                    filename, mode="w+", dtype=np.float64, shape=covariance_shape
                )
                ...
                gene_iterator = tqdm(
                    range(n_genes), desc="Computing covariance", disable=not progress
                )
            ...
            if self._compute_cov_slice_jit is None:
                ...
                for g in gene_iterator:
                    gene_centered_np = centered_reshaped_np[
                        :, :, g
                    ]  # (n_cells, n_groups)
