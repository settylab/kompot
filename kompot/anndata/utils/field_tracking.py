    ...
    if not isinstance(run_history, list):
        if isinstance(run_history, str):
            try:
                ...
                if not isinstance(run_history, list):
                    logger.warning(
                        f"Parsed run_history is not a list for {analysis_type}: {type(run_history)}"
                    )
                    return []
            except Exception:
                logger.warning(
                    f"Failed to parse run_history as JSON for {analysis_type}"
                )
                return []
        else:
    ...
    for item in run_history:
        if isinstance(item, str):
            try:
                ...
                logger.warning(
                    f"Failed to parse run history item as JSON: {item[:50]}..."
                )
                # Skip invalid items
                continue
...
def append_to_run_history(
    adata: AnnData, run_info: Dict[str, Any], analysis_type: str = "da"
) -> bool:
    """
...
def get_last_run_info(
    adata: AnnData, analysis_type: str = "da"
...
def generate_output_field_names(
    ...
    if analysis_type == "da":
        ...
        sample_variance_impacted = [
            "mahalanobis_key",
            "ptp_key",
            "lfc_std_key",
            "mahalanobis_varm_key",
            "std_key_1",
            "std_key_2",
            "fold_change_zscores_key",
        ]
        ...
        field_names["posterior_covariance_key"] = (
            f"{result_key}_{cond1_safe}_to_{cond2_safe}_posterior_covariance"
        )
        ...
        field_names["all_patterns"] = {
            ...
            "obsp": [
                field_names[
                    "posterior_covariance_key"
                ]  # Not impacted by sample variance
            ],
        }
    ...
    **kwargs,  # Accept additional arguments for backward compatibility
    ...
    if result_key is not None:
        ...
        if (
            ...
            and "anndata_fields" in adata.uns[storage_key]
            and "uns" in adata.uns[storage_key]["anndata_fields"]
        ):
...
def validate_field_run_id(
    ...
    if storage_key in adata.uns and "anndata_fields" in adata.uns[storage_key]:
        ...
        if location in anndata_fields:
            ...
            if field_name in tracking_info:
                ...
                if actual_run_id != requested_run_id:
                    warning_msg = (
                        f"Field '{field_name}' in {location} was last written by run_id={actual_run_id}, "
    ...
    if not isinstance(run_info, dict):
        if isinstance(run_info, str):
            ...
            try:
                ...
                if not isinstance(run_info, dict):
                    logger.warning(
                        f"Expected run_info to be a dict, but got {type(run_info)}"
                    )
