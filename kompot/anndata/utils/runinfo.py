...
from .field_tracking import get_run_from_history
...
class RunInfo:
    ...
    def __init__(
        self, adata, run_id: Optional[int] = None, analysis_type: Optional[str] = None
    ):
        """
        ...
        if (
            self.storage_key not in adata.uns
            or "run_history" not in adata.uns[self.storage_key]
            or len(adata.uns[self.storage_key]["run_history"]) == 0
        ):
            raise ValueError(f"No run history found for {analysis_type} analysis.")

        # Get run info
        self.run_info = get_run_from_history(
            adata, run_id=run_id, analysis_type=analysis_type
        )

        if self.run_info is None:
            raise ValueError(
                f"Run ID {run_id} not found in {analysis_type} run history."
            )
    ...
    def _check_overwritten_fields(self) -> List[Dict[str, Any]]:
        ...
        """
        if (
            self.storage_key not in self.adata.uns
            or "anndata_fields" not in self.adata.uns[self.storage_key]
        ):
            return []
        ...
        if not field_mapping:
            logger.warning(
                f"No field_mapping found for run {self.adjusted_run_id} to check for overwritten fields."
            )
            return []
        ...
        for field, mapping in field_mapping.items():
            ...
            if (
                not location
                or location not in tracking
                or field not in tracking[location]
            ):
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            if hasattr(self, "overwritten_fields")
            else 0,
            "overwritten_fields": self.overwritten_fields
            if hasattr(self, "overwritten_fields")
            else [],
            "missing_field_count": len(self.missing_fields)
            if hasattr(self, "missing_fields")
            else 0,
    ...
    def _repr_html_(self) -> str:
        ...
        if summary.get("missing_field_count", 0) > 0:
            html.append(
                f"<span class='badge badge-danger'>Fields Missing: {summary['missing_field_count']}</span>"
            )
        if summary.get("overwritten_field_count", 0) > 0:
            html.append(
                f"<span class='badge badge-warning'>Fields Overwritten: {summary['overwritten_field_count']}</span>"
            )
        if (
            summary.get("overwritten_field_count", 0) == 0
            and summary.get("missing_field_count", 0) == 0
        ):
        ...
        html.append("<table>")
        html.append(
            "<tr><th style='width:30%'>Parameter</th><th style='width:70%'>Value</th></tr>"
        )
        ...
        # Field counts
        html.append(
            f"<tr><td>Fields Created</td><td>{summary.get('field_count', 0)}</td></tr>"
        )

        # Group information if available
        if summary.get("has_groups", False):
            html.append(
                f"<tr><td>Groups</td><td>{summary.get('groups', '')} ({summary.get('groups_count', 0)} total)</td></tr>"
            )
        ...
        if self.environment:
            ...
            for k, v in sorted(self.environment.items()):
                ...
                if k == "package_versions" and isinstance(v, dict):
                    version_str = "<br>".join(
                        [f"&nbsp;&nbsp;{pkg}: {ver}" for pkg, ver in sorted(v.items())]
                    )
        ...
        if self.adata_fields:
            ...
            for location, fields in self.adata_fields.items():
                for field in fields:
                    ...
                    overwritten_info = next(
                        (
                            info
                            for info in self.overwritten_fields
                            if info["location"] == location and info["field"] == field
                        ),
                        None,
                    )
                    if overwritten_info:
                    ...
                    missing_info = next(
                        (
                            info
                            for info in self.missing_fields
                            if info["location"] == location and info["field"] == field
                        ),
                        None,
                    )
            ...
            for location in sorted(fields_by_location.keys()):
                ...
                # Add location header
                html.append(
                    f"<tr><td colspan='4' class='location-header'>{location.upper()} Fields</td></tr>"
                )
...
class RunComparison:
    ...
    def __init__(self, adata: AnnData, run_id1: int, run_id2: int, analysis_type: str):
        """
    ...
    def _compare_fields(self) -> Dict[str, Any]:
        ...
        for location in all_locations:
            ...
            comparison["by_location"][location]["same"] = sorted(
                list(fields_set1.intersection(fields_set2))
            )
            comparison["by_location"][location]["only_in_run1"] = sorted(
                list(fields_set1 - fields_set2)
            )
            comparison["by_location"][location]["only_in_run2"] = sorted(
                list(fields_set2 - fields_set1)
            )

        # Add totals
        total_same = sum(
            len(data["same"]) for data in comparison["by_location"].values()
        )
        total_only_in_run1 = sum(
            len(data["only_in_run1"]) for data in comparison["by_location"].values()
        )
        total_only_in_run2 = sum(
            len(data["only_in_run2"]) for data in comparison["by_location"].values()
        )
    ...
    def __repr__(self) -> str:
        ...
        summary = self.get_summary()
        return (
            f"RunComparison(run1={summary['run1']['run_id']}, "
    ...
    def _repr_html_(self) -> str:
        ...
        total_params = (
            len(self.param_comparison["same"])
            + len(self.param_comparison["different"])
            + len(self.param_comparison["only_in_run1"])
        ...
        param_badge_class = (
            "badge-success" if different_params == 0 else "badge-warning"
        )
        html.append(
            f"<span class='badge {param_badge_class}'>Parameters: {different_params} different, {same_params} same</span>"
        )
        ...
        total_fields = (
            field_totals["same"]
            + field_totals["only_in_run1"]
            + field_totals["only_in_run2"]
        )
        different_fields = field_totals["only_in_run1"] + field_totals["only_in_run2"]

        field_badge_class = (
            "badge-success" if different_fields == 0 else "badge-warning"
        )
        html.append(
            f"<span class='badge {field_badge_class}'>Fields: {different_fields} different, {field_totals['same']} same</span>"
        )
        ...
        html.append("<table>")
        html.append(
            "<tr><th rowspan='2'>Aspect</th><th colspan='2'>Run Details</th></tr>"
        )
        html.append(
            f"<tr><th>Run {self.run1.adjusted_run_id}</th><th>Run {self.run2.adjusted_run_id}</th></tr>"
        )
        ...
        param_badges = []
        if (
            "condition1" in self.param_comparison["different"]
            or "condition2" in self.param_comparison["different"]
        ):
            param_badges.append(
                "<span class='badge badge-danger'>Different Conditions</span>"
            )
        if "groupby" in self.param_comparison["different"]:
            param_badges.append(
                "<span class='badge badge-danger'>Different Group Variable</span>"
            )
        if "obsm_key" in self.param_comparison["different"]:
            param_badges.append(
                "<span class='badge badge-danger'>Different Embedding</span>"
            )
        if "layer" in self.param_comparison["different"]:
            param_badges.append(
                "<span class='badge badge-danger'>Different Layer</span>"
            )
        if "use_sample_variance" in self.param_comparison["different"]:
            param_badges.append(
                "<span class='badge badge-warning'>Different Variance Method</span>"
            )
        ...
        html.append(
            f"<strong>Different:</strong> {len(self.param_comparison['different'])} &nbsp;|&nbsp; "
        )
        html.append(
            f"<strong>Only in Run {self.run1.adjusted_run_id}:</strong> {len(self.param_comparison['only_in_run1'])} &nbsp;|&nbsp; "
        )
        html.append(
            f"<strong>Only in Run {self.run2.adjusted_run_id}:</strong> {len(self.param_comparison['only_in_run2'])}"
        )
        ...
        if key_param_diffs:
            ...
            html.append(
                "<h5 style='margin-top:15px; color:#d32f2f;'>Key Parameter Differences</h5>"
            )
            html.append("<table>")
            html.append("<tr>")
            html.append("<th style='width:30%'>Parameter</th>")
        ...
        if (
            self.param_comparison["different"]
            or self.param_comparison["only_in_run1"]
            or self.param_comparison["only_in_run2"]
        ):
            ...
            if self.param_comparison["different"]:
                html.append(
                    "<tr><td colspan='3' class='field-category diff-changed'>Different Parameters</td></tr>"
                )
            ...
            if self.param_comparison["only_in_run1"]:
                html.append(
                    f"<tr><td colspan='3' class='field-category diff-removed'>Only in Run {self.run1.adjusted_run_id}</td></tr>"
                )
                for param, value in sorted(
                    self.param_comparison["only_in_run1"].items()
                ):
                    html.append("<tr class='diff-removed'>")
                    html.append(f"<td>{param}</td>")
                    html.append(f"<td>{value}</td>")
                    html.append("<td class='not-set'>not set</td>")
                    html.append("</tr>")

            # Add parameters only in run2
            if self.param_comparison["only_in_run2"]:
                html.append(
                    f"<tr><td colspan='3' class='field-category diff-added'>Only in Run {self.run2.adjusted_run_id}</td></tr>"
                )
                for param, value in sorted(
                    self.param_comparison["only_in_run2"].items()
                ):
            ...
            if len(self.param_comparison["same"]) <= 5:
                html.append(
                    "<tr><td colspan='3' class='field-category diff-unchanged'>Same Parameters</td></tr>"
                )
                ...
                html.append(
                    "<tr><td colspan='3' class='field-category diff-unchanged'>"
                )
                html.append(
                    f"{len(self.param_comparison['same'])} parameters are the same in both runs"
                )
                html.append("</td></tr>")
        ...
        html.append("<div class='summary-box'>")
        html.append(
            f"<strong>Fields in both runs:</strong> {self.field_comparison['totals']['same']} &nbsp;|&nbsp; "
        )
        html.append(
            f"<strong>Only in Run {self.run1.adjusted_run_id}:</strong> {self.field_comparison['totals']['only_in_run1']} &nbsp;|&nbsp; "
        )
        html.append(
            f"<strong>Only in Run {self.run2.adjusted_run_id}:</strong> {self.field_comparison['totals']['only_in_run2']}"
        )
        html.append("</div>")
        ...
        if (
            storage_key in self.adata.uns
            and "anndata_fields" in self.adata.uns[storage_key]
        ):
            from .json_utils import from_json_string
        ...
        for location in sorted(fields_by_location.keys()):
            ...
            if loc_data["same"]:
                ...
                html.append(
                    f"<tr><td colspan='4' class='location-header-shared'>{location.upper()} Shared Fields</td></tr>"
                )
                ...
                for field in sorted(loc_data["same"]):
                    ...
                    if (
                        location in field_ownership
                        and field in field_ownership[location]
                    ):
        ...
        for location in sorted(fields_by_location.keys()):
            ...
            if loc_data["only_in_run1"] or loc_data["only_in_run2"]:
                ...
                # Display location header
                html.append(
                    f"<tr><td colspan='4' class='location-header'>{location.upper()} Different Fields</td></tr>"
                )
                ...
                for field in sorted(loc_data["only_in_run1"]):
                    ...
                    html.append(
                        f"<td class='run1-only'>Run {self.run1.adjusted_run_id}</td>"
                    )
                    html.append("</tr>")
                ...
                for field in sorted(loc_data["only_in_run2"]):
                    ...
                    html.append(
                        f"<td class='run2-only'>Run {self.run2.adjusted_run_id}</td>"
                    )
                    html.append("</tr>")
        ...
        if has_overlapping_fields:
            ...
            html.append(
                "<strong>Note on shared fields:</strong> When both runs define the same field, the last run to write to the field "
            )
            html.append(
                "overwrites the previous value. The 'Last Modified By' column shows which run's value is currently stored."
            )
            html.append("</div>")
