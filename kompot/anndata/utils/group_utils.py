...
def parse_groups(adata, groups, formatted_names=False, return_description=False):
            ...
            for filter_name, filter_dict in groups.items():
                ...
                for col_name, value in filter_dict.items():
                    ...
                    if isinstance(value, (str, bool)):
                        ...
                        filter_parts.append(
                            f"{col_name}={'+'.join(str(v) for v in value)}"
                        )
                    else:
                        raise ValueError(
                            f"Unsupported value type for column '{col_name}': {type(value)}"
                        )
            ...
            for group_name, group_value in groups.items():
                ...
                if isinstance(group_value, str):
                    ...
                    if group_name in adata.obs.columns:
                        ...
                        if not np.any(mask):
                            raise ValueError(
                                f"Value '{group_value}' not found in any column"
                            )
                        subset_name = name
                # Handle list of strings for column values
                elif isinstance(group_value, list) and all(
                    isinstance(v, str) for v in group_value
                ):
                    ...
                    if group_name in adata.obs.columns:
                        ...
                        # Not supported
                        raise ValueError(
                            f"List values are only supported for column names, not '{group_name}'"
                        )
                # Handle True/False boolean values
                    ...
                    if group_name in adata.obs.columns:
                        ...
                        raise ValueError(
                            f"Boolean value for '{group_name}' requires a boolean column in adata.obs"
                        )
                # Convert value to boolean mask if it's an array-like object
                    ...
                    # If it's already a boolean mask, ensure it's a numpy array
                        ...
                        try:
                            ...
                            raise ValueError(
                                f"Could not convert group '{name}' value to boolean mask: {e}"
                            )
                else:
                    raise ValueError(
                        f"Group '{name}' value must be list, array, Series, or str, got {type(group_value)}"
                    )

                # Verify mask length
                if len(mask) != adata.n_obs:
                    raise ValueError(
                        f"Group '{name}' mask length ({len(mask)}) doesn't match adata.n_obs ({adata.n_obs})"
                    )
    ...
    # Case 3: boolean mask or categorical pandas Series
        ...
        if len(groups) != adata.n_obs:
            raise ValueError(
                f"Length of groups Series ({len(groups)}) doesn't match adata.n_obs ({adata.n_obs})"
            )
    ...
    # Case 4: numpy array boolean mask or indices
            ...
            for i in range(groups.shape[0]):
                ...
                if mask.dtype != bool:
                    try:
                        ...
                        raise ValueError(
                            f"Could not convert row {i} to boolean mask: {e}"
                        )
        ...
        # Regular 1D array case
            ...
            if groups.dtype == bool:
                ...
                if len(groups) != adata.n_obs:
                    raise ValueError(
                        f"Length of boolean mask ({len(groups)}) doesn't match adata.n_obs ({adata.n_obs})"
                    )
                ...
                np.issubdtype(groups.dtype, np.character)
                or groups.dtype.kind == "U"
                or groups.dtype.kind == "S"
            ):
                # String/categorical array - similar to Series handling
                if len(groups) != adata.n_obs:
                    raise ValueError(
                        f"Length of categorical array ({len(groups)}) doesn't match adata.n_obs ({adata.n_obs})"
                    )
                ...
                raise ValueError(
                    f"Numpy array groups must be boolean, integer, or string type, got {groups.dtype}"
                )
        else:
            raise ValueError(
                f"Array shape {groups.shape} not supported. Expected 1D array of length {adata.n_obs} or 2D array with shape (n_masks, {adata.n_obs})"
            )
    ...
    # Case 5: list of arrays
        ...
        # Check if it's a list of arrays
            ...
            for i, array in enumerate(groups):
                ...
                if array.dtype == bool:
                    ...
                    if len(array) != adata.n_obs:
                        raise ValueError(
                            f"Length of boolean mask ({len(array)}) doesn't match adata.n_obs ({adata.n_obs})"
                        )
                    ...
                    np.issubdtype(array.dtype, np.character)
                    or array.dtype.kind == "U"
                    or array.dtype.kind == "S"
                ):
                    # String/categorical array
                    if len(array) != adata.n_obs:
                        raise ValueError(
                            f"Length of categorical array ({len(array)}) doesn't match adata.n_obs ({adata.n_obs})"
                        )
                    ...
                    if len(array) != adata.n_obs:
                        raise ValueError(
                            f"Length of numeric array ({len(array)}) doesn't match adata.n_obs ({adata.n_obs})"
                        )
            ...
            raise ValueError(
                f"List items must all be dictionaries or arrays, got mixed types: {[type(item) for item in groups]}"
            )
    else:
...
def check_underrepresentation(
    ...
    result = {"__underrepresentation_data": underrepresentation_data}
    ...
    if print_summary and underrepresentation_data:
        ...
        print(
            f"Found {len(underrepresentation_data)} groups with underrepresented conditions."
        )
        ...
        for group, conditions_list in underrepresentation_data.items():
            ...
            for condition in conditions:
                ...
                print(
                    f"  {condition:<20} {cells_in_condition:<10} {percentage:.2f}%{' ' * 9} {status:<15}"
                )
        ...
        logger.info(
            "The detected underrepresentation may affect differential expression results."
        )
        logger.info(
            "Consider filtering these groups with the returned dictionary when running differential analysis:"
        )
        logger.info(
            "Example: adata.compute_differential_expression(..., cell_filter=underrepresented)"
        )
        logger.info(f"{'=' * 80}\n")
...
def apply_cell_filter(
    ...
    filter_details = {
        ...
        else str(type(cell_filter).__name__),
        "auto_filtered": False,
    ...
    if cell_filter is None:
        ...
        if filter_col.dtype == bool:
            ...
            try:
                ...
                raise ValueError(
                    f"Could not convert column '{cell_filter}' to boolean mask"
                )
        ...
        if cell_filter.dtype == bool:
            ...
            if len(cell_filter) != adata.n_obs:
                raise ValueError(
                    f"Length of boolean mask ({len(cell_filter)}) doesn't match adata.n_obs ({adata.n_obs})"
                )
            ...
            raise ValueError(
                f"Array filter must be boolean or integer, got {cell_filter.dtype}"
            )
    else:
    ...
    if check_representation and groupby is not None and groups is not None:
        ...
        # Get refined mask
        refined_mask, underrep_data, excluded_count = (
        ...
        if excluded_count > 0:
            if verbosity > 0:
                logger.info(
                    f"Automatically filtering out {excluded_count} cells from groups with underrepresentation"
                )
    ...
    filter_details["filter_percentage"] = (
        100 * filter_details["filtered_cells"] / filter_details["total_cells"]
    )

    if verbosity > 0:
        if filter_details["filtered_cells"] < filter_details["total_cells"]:
            logger.info(
                f"Using {filter_details['filtered_cells']} of {filter_details['total_cells']} cells ({filter_details['filter_percentage']:.1f}%)"
            )
        else:
            logger.debug(
                f"Using {filter_details['filtered_cells']} of {filter_details['total_cells']} cells ({filter_details['filter_percentage']:.1f}%)"
