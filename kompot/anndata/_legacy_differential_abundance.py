    ...
    **density_kwargs : dict
    ...
    if landmarks is None:
        ...
        if (
            ...
            if landmarks_dim == data_dim:
                logger.info(
                    f"Reusing stored landmarks from adata.uns['{storage_key}']['landmarks'] with shape {other_landmarks.shape}"
                )
                landmarks = other_landmarks
            else:
                logger.warning(
                    f"Other stored landmarks have dimension {landmarks_dim} but data has dimension {data_dim}. Will check for other landmarks."
                )
    ...
    if landmarks is None:
        for key in adata.uns.keys():
            if (
                ...
                if landmarks_dim == data_dim:
                    logger.info(
                        f"Reusing landmarks from adata.uns['{key}']['landmarks'] with shape {check_landmarks.shape}"
                    )
                    landmarks = check_landmarks
                    break
                else:
                    logger.debug(
                        f"Found landmarks in {key} but dimensions don't match: {landmarks_dim} vs {data_dim}"
                    )

    # If still no landmarks, specifically check for DE landmarks for backward compatibility
    if (
        landmarks is None
        and "kompot_de" in adata.uns
        and "landmarks" in adata.uns["kompot_de"]
    ):
        ...
        if landmarks_dim == data_dim:
            logger.info(
                f"Reusing differential expression landmarks from adata.uns['kompot_de']['landmarks'] with shape {de_landmarks.shape}"
            )
            landmarks = de_landmarks
        else:
            logger.warning(
                f"DE landmarks have dimension {landmarks_dim} but data has dimension {data_dim}. Computing new landmarks."
            )
    ...
    if (
        ...
        landmarks_info = {
            "shape": list(
                landmarks_shape
            ),  # Convert tuple to list for HDF5 serialization
    ...
    ]  # Now using negative log10 PTPs (Posterior Tail Probabilities) (higher = more significant)
