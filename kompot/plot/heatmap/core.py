...
def heatmap(
    ...
    layout_config: Optional[
        Dict[str, float]
    ] = None,  # Configuration for layout spacing
    ...
    max_cell_count: Optional[
        int
    ] = None,  # Upper limit for cell count used for dot sizing (None = use actual max)
    **kwargs,
):
    """
    ...
    **kwargs :
    ...
    if run_info is not None and "params" in run_info:
        ...
        if condition_column is None:
            ...
            logger.info(
                f"Inferred condition_column='{condition_column}' from run information"
            )
        if condition1 is None or condition2 is None:
            ...
            if "condition1" in params and "condition2" in params:
                if condition1 is None:
                    ...
                    logger.info(
                        f"Inferred condition1='{condition1}' from run information"
                    )
                if condition2 is None:
                    condition2 = params["condition2"]
                    logger.info(
                        f"Inferred condition2='{condition2}' from run information"
                    )
    ...
    if fold_change_mode:
        logger.info(
            f"Creating fold change heatmap with {len(var_names)} genes/features"
        )
    elif split_dot_mode:
    ...
    if condition_column is None:
        logger.warning(
            "No condition_column could be inferred. Split heatmap requires a condition column."
        )
        return None
    ...
    if condition1 is None:
        logger.error(
            "No value for condition1 could be inferred. Please provide using condition1 parameter."
        )
        return None
    if condition2 is None:
        logger.error(
            "No value for condition2 could be inferred. Please provide using condition2 parameter."
        )
        return None
    ...
    # Split by condition
    cond1_df = expr_df[expr_df[condition_column] == condition1].drop(
        columns=[condition_column]
    )
    cond2_df = expr_df[expr_df[condition_column] == condition2].drop(
        columns=[condition_column]
    )
    ...
    if exclude_groups is not None:
        ...
        cond1_df = _filter_excluded_groups(
            cond1_df, groupby, exclude_groups, all_groups
        )
        cond2_df = _filter_excluded_groups(
            cond2_df, groupby, exclude_groups, all_groups
        )

        # Update all_groups list after filtering
        all_groups = sorted(
            set(cond1_df[groupby].unique()) | set(cond2_df[groupby].unique())
        )
    ...
    if split_dot_mode:
        ...
        if max_cell_count is not None and max_cell_count > 0:
            ...
            logger.info(
                f"Using user-specified max count limit of {global_max_count} cells to scale all dots (actual max: {int(actual_max_count)})"
            )
        else:
            global_max_count = actual_max_count
            logger.info(
                f"Using global max count of {int(global_max_count)} cells to scale all dots"
            )
    ...
    if standard_scale is not None:
        ...
        scaled = _apply_scaling(
            combined, standard_scale, is_split=True, has_hierarchical_index=True
        )
    ...
    if figsize is None:
        ...
        GROUP_LABEL_SPACE = layout[
            "group_label_space"
        ]  # Space for group labels (x-axis)
        TITLE_SPACE = layout["title_space"]  # Space for title

        # Dynamically determine legend space based on condition name lengths
        max_condition_name_length = max(
            len(str(condition1_name or "")), len(str(condition2_name or ""))
        )

        # Calculate additional space needed for the legend based on name length
        additional_space = (max_condition_name_length + 5) * layout[
            "legend_name_factor"
        ]
        ...
        ROW_DENDROGRAM_SPACE = layout[
            "row_dendrogram_space"
        ]  # Space for row dendrogram
        COL_DENDROGRAM_SPACE = layout[
            "col_dendrogram_space"
        ]  # Space for column dendrogram
        ...
        left_area_inches = (
            GENE_LABEL_SPACE * base_unit if show_gene_labels else base_unit
        )

        # Bottom area - for group labels
        bottom_area_inches = (
            GROUP_LABEL_SPACE * base_unit if show_group_labels else base_unit
        )
        ...
        legend_factor = 1.0 + max(
            0, (max_condition_name_length * layout["legend_name_factor"])
        )

        # Approximate reasonable areas based on base_unit
        left_area_inches = min(
            layout["gene_label_space"] * base_unit, width_inches * 0.2
        )
        right_area_inches = min(
            layout["base_legend_space"] * base_unit * legend_factor, width_inches * 0.3
        )
        bottom_area_inches = min(
            layout["group_label_space"] * base_unit, height_inches * 0.15
        )
        top_area_inches = min(layout["title_space"] * base_unit, height_inches * 0.15)
        if dendrogram:
            if cluster_cols:
                top_area_inches += min(
                    layout["row_dendrogram_space"] * base_unit, height_inches * 0.1
                )
            if cluster_rows:
                right_area_inches += min(
                    layout["col_dendrogram_space"] * base_unit, width_inches * 0.1
                )
    ...
    if create_fig:
        ...
        if dendrogram:
            ...
            if cluster_rows:
                col_dend_width = (
                    COL_DENDROGRAM_SPACE * fig_dims["base_unit"]
                ) / fig_width
                col_dend_left = (
                    main_left + main_width + (0.5 * fig_dims["base_unit"] / fig_width)
                )
                col_dendrogram_ax = fig.add_axes(
                    [col_dend_left, main_bottom, col_dend_width, main_height]
                )
            ...
            if cluster_cols:
                row_dend_height = (
                    ROW_DENDROGRAM_SPACE * fig_dims["base_unit"]
                ) / fig_height
                row_dend_bottom = (
                    main_bottom
                    + main_height
                    + (0.5 * fig_dims["base_unit"] / fig_height)
                )
                row_dendrogram_ax = fig.add_axes(
                    [main_left, row_dend_bottom, main_width, row_dend_height]
                )
        ...
        if fig_dims["has_row_dendrogram"]:
            sidebar_left += (
                COL_DENDROGRAM_SPACE * fig_dims["base_unit"] + fig_dims["base_unit"]
            ) / fig_width
        else:
            sidebar_left += (0.5 * fig_dims["base_unit"]) / fig_width

        sidebar_width = (LEGEND_SPACE * fig_dims["base_unit"]) / fig_width
        sidebar_ax = fig.add_axes(
            [sidebar_left, main_bottom, sidebar_width, main_height]
        )
    ...
    if cluster_rows or cluster_cols:
        ...
        if cluster_cols:
            ...
            if dendrogram and "row" in dendrogram_axes:
                ...
                temp_tree = scipy_dendrogram(row_linkage_matrix, no_plot=True)
                row_order = temp_tree["leaves"]

            # Make sure we don't have empty cluster issue
            if len(row_order) != cond1_means.shape[0]:
                logger.warning(
                    f"Mismatch in row_order length ({len(row_order)}) vs. data rows ({cond1_means.shape[0]})"
                )
        ...
        if cluster_rows:
            ...
            col_dist = ssd.pdist(
                combined_cols_for_clustering.values.T
            )  # Transpose for column distance
            ...
            if dendrogram and "col" in dendrogram_axes:
                ...
                # Just get the leaf order without drawing
                temp_tree = scipy_dendrogram(col_linkage_matrix, no_plot=True)
            ...
            if len(col_order_indices) != n_columns:
                logger.warning(
                    f"Column order mismatch detected: {len(col_order_indices)} vs {n_columns} - fixing"
                )
    ...
    if fold_change_mode:
        ...
        all_data = np.concatenate(
            [cond1_means.values.flatten(), cond2_means.values.flatten()]
        )
    ...
    is_zscored = not fold_change_mode and (
        standard_scale == "var"
        or standard_scale == 0
        or standard_scale == "group"
        or standard_scale == 1
    )
    ...
    if fold_change_mode and standard_scale is not None:
        logger.warning(
            "standard_scale is ignored in fold_change_mode as z-scoring is not appropriate for fold changes"
        )
        ...
        if isinstance(value, str) and value.startswith("p"):
            try:
                ...
                if 0 <= percentile <= 100:
                    ...
                    logger.warning(
                        f"Invalid percentile {percentile}, must be between 0 and 100. Using None instead."
                    )
                    return None
            except ValueError:
                logger.warning(
                    f"Invalid percentile format '{value}'. Use 'p<number>' (e.g., 'p5'). Using None instead."
                )
                return None
        return value
    ...
    if (is_zscored or fold_change_mode) and parsed_vmin is None and parsed_vmax is None:
        ...
        if len(all_data) > 0:
            ...
            if fold_change_mode:
                logger.info(
                    f"Using symmetric colormap limits [-{abs_max:.2f}, {abs_max:.2f}] for fold change data"
                )
            else:
                logger.info(
                    f"Using symmetric colormap limits [-{abs_max:.2f}, {abs_max:.2f}] for z-scored data"
                )
        else:
            ...
            effective_vmax = 1.0
            logger.info(
                "No valid data found, using default colormap limits [-1.0, 1.0]"
            )
    ...
    if (
        effective_vmin is not None
        and effective_vmax is not None
        and effective_vmin >= effective_vmax
    ):
    ...
    if show_group_labels:
        ...
        ax.set_xticklabels(
            cond1_means.columns, rotation=90, fontsize=group_labels_size, ha="center"
        )
    ...
    if create_fig:  # Only if we created the figure
        ...
        if title:
            ...
            title_text = (
                f"{condition1_name} to {condition2_name}\nMean expression by {groupby}"
            )
        else:
    ...
    if create_fig:  # Only if we created the figure
        ...
        if fold_change_mode:
            ...
            positive_change = mpatches.Rectangle(
                ...
                facecolor=cmap_obj(
                    0.7
                ),  # Use a specific color for legend (higher values)
            ...
            negative_change = mpatches.Rectangle(
                ...
                facecolor=cmap_obj(
                    0.3
                ),  # Use a different color for contrast (lower values)
            ...
            if not non_zero_counts:
                ...
                max_count = (
                    global_max_count
                    if "global_max_count" in locals()
                    else max(non_zero_counts)
                )
            ...
            if (
                max_cell_count is not None
                and actual_max_count > max_cell_count
                and large_count >= max_cell_count
            ):
            ...
            small_example = mpatches.Rectangle(
                (0, 0), 1, 1, fill=False, edgecolor="none", label=small_label
            )
            medium_example = mpatches.Rectangle(
                (0, 0), 1, 1, fill=False, edgecolor="none", label=medium_label
            )
            large_example = mpatches.Rectangle(
                (0, 0), 1, 1, fill=False, edgecolor="none", label=large_label
            )
        ...
        if split_dot_mode:
            ...
            legend_height = 1.0 - (
                layout["colorbar_height"] if layout else 0.4
            )  # Use top portion for legend
        ...
        if split_dot_mode:
            ...
            if "large_label" in locals():
                max_cell_label_length = max(
                    len(small_label), len(medium_label), len(large_label)
                )
        ...
        adaptive_fontsize = max(
            8, base_fontsize - max(0, max_text_length * fontsize_factor)
        )
        ...
        if fold_change_mode:
            ...
            class SizeTextHandler(HandlerBase):
                ...
                def create_artists(
                    ...
                    scale_factor = max_radius / np.sqrt(
                        global_max_count
            ...
            for item in legend_elements:
                if any(
                    label in item.get_label()
                    for label in [small_label, medium_label, large_label]
                ):
            ...
            if split_dot_mode:
                ...
                dot_size_elements = [small_example, medium_example, large_example]

                # Create handler maps for each legend
                condition_handler_map = {mpatches.Wedge: HandlerWedge()}
                ...
                legend_area_height = bbox.height * (
                    1 - colorbar_height - 0.05
                )  # 5% buffer
                ...
                max_radius = (
                    ...
                    * kompot_legend_dot_info["max_size_factor"]
                    / 2
                )
                ...
                dynamic_scale = SizeTextHandler.calculate_scale_factor(
                    adaptive_fontsize
                )
                ...
                dynamic_spacing = (largest_dot_radius * 0.5 / adaptive_fontsize) + 1.0
                ...
                dot_sizes_legend = dot_sizes_ax.legend(
                    ...
                    labelspacing=max(
                        1.5, dynamic_spacing
                    ),  # Use dynamic spacing with a minimum value
                )
        ...
        colorbar_height = (
            layout["colorbar_height"] if layout else 0.5
        )  # Use configured height for sidebar
        ...
        colorbar_width = (
            layout["colorbar_width"] if layout else 0.25
        )  # Width from layout config
        colorbar_ax = fig.add_axes(
            [
                bbox.x0
                + bbox.width * (0.5 - colorbar_width / 2),  # Center horizontally
                bbox.y0,  # Start from bottom of sidebar
                bbox.width * colorbar_width,  # Make narrower for smaller colorbar
                bbox.height
                * (1 - legend_height - gap),  # Fill remaining space minus gap
            ]
        )
    ...
    if colorbar_kwargs:
        ...
        for key, value in colorbar_kwargs.items():
            if key not in ["label_kwargs", "locator", "formatter"] and hasattr(
                cbar, key
            ):
    ...
    if return_data:
        HeatmapResult = namedtuple(
            "HeatmapResult", ["fig", "cond1_means", "cond2_means", "fold_changes"]
        )
