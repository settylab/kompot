...
def multi_volcano_da(
    ...
    background_kwargs: Optional[
        Dict[str, Any]
    ] = None,  # Additional kwargs for the background plot
    ...
    **kwargs :
    ...
    if needed_column_inference:
        logger.info(
            f"Inferred columns for multi-volcano plot: lfc_key='{lfc_key}', ptp_key='{ptp_key}'"
        )
    ...
    total_width_units = (
        layout["y_label_width"]
        + layout["y_label_offset"]
        + layout["plot_width"]
        + layout["label_width"]
    )
    plot_area_height = (layout["plot_height"] * n_groups) + (
        layout["plot_spacing"] * (n_groups - 1)
    )
    ...
    legend_area_height_norm = (
        (legend_height + layout["legend_bottom_margin"]) * unit / fig_height
    )
    main_bottom = legend_area_height_norm + (
        layout["legend_plot_gap"] * unit / fig_height
    )
    main_top = main_bottom + plot_area_height_norm

    # Calculate left position for the main plot area (after y-label)
    y_label_width_norm = (
        (layout["y_label_width"] + layout["y_label_offset"]) * unit / fig_width
    )
        ...
        if i == 0:
            ...
            if share_y:
                plot_ax = fig.add_subplot(
                    gs[row_idx, 0], sharex=shared_x, sharey=shared_y
                )
            else:
    ...
    if ylabel and n_groups > 0:
        ...
        y_label_ax = fig.add_axes(
            [
                y_label_left,
                y_center - (y_label_height / 2),
                y_label_width,
                y_label_height,
            ]
        )
    ...
    if (
        ...
        and not pd.api.types.is_categorical_dtype(adata.obs[color])
    ):
    ...
    for i, group in enumerate(groups):
        ...
        if background_plot is not None:
            ...
            plot_height = (
                background_height_factor * global_y_range
            )  # Height for KDE/violin
            ...
            if background_plot == "kde":
                ...
                if bg_kwargs.get("show_2d_kde", False):
                    try:
                        ...
                        y_grid = np.linspace(
                            y_min - 0.2 * y_range, y_max + 0.2 * y_range, 100
                        )
                        ...
                        kernel = stats.gaussian_kde(
                            values, bw_method=bg_kwargs.get("bw_method")
                        )
                        density = np.reshape(kernel(positions).T, xx.shape)

                        # Plot 2D KDE as contour with low alpha
                        contour_levels = bg_kwargs.get("contour_levels", 5)
                        contour_alpha = bg_kwargs.get(
                            "contour_alpha", background_alpha * 0.5
                        )
                        plot_ax.contourf(
                            xx,
                            yy,
                        ...
                        logger.warning(
                            f"Failed to create 2D KDE contour plot: {e}. Skipping."
                        )
                ...
                if not bg_kwargs.get("showmeans", False) and not bg_kwargs.get(
                    "showmedians", False
                ):
            ...
            for c in color_values:
                ...
                if pd.api.types.is_categorical_dtype(adata.obs[c]):
                    ...
                    if vcenter is not None:
                        ...
                        scatter_kwargs_color["norm"] = TwoSlopeNorm(
                            vmin=use_vmin, vcenter=vcenter, vmax=use_vmax
                        )
                    else:
        ...
        if show_thresholds:
            if lfc_threshold is not None:
                plot_ax.axvline(
                    x=lfc_threshold, color="black", linestyle="--", alpha=0.5
                )
                plot_ax.axvline(
                    x=-lfc_threshold, color="black", linestyle="--", alpha=0.5
                )
    ...
    if show_legend is not None:
        ...
        if isinstance(color, str) and color == groupby:
            logger.info(
                f"Color column '{color}' is identical to groupby column - not showing legend"
            )
    ...
    if show_legend_now and (len(all_handles) > 0 or colorbar_needed):
        ...
        if colorbar_needed and first_color_mappable is not None:
            actual_legend_height = max(
                layout["legend_height"], 3
            )  # Minimum 3 units for colorbar
        ...
        legend_top = last_plot_bottom - (
            (legend_gap + additional_gap) * unit / fig_height
        )
        ...
        plot_right = (
            0.95 - 0.15
        )  # The right edge of the plot area (minus the label width)
