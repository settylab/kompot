...
def volcano_de(
    ...
    **kwargs :
    ...
    if group is not None:
        ...
        logger.debug(
            f"Using varm keys: lfc={lfc_varm_key}, score={score_varm_key}, weighted={weighted_lfc_varm_key}"
        )
        ...
        weighted_lfc_data_available = (
            weighted_lfc_varm_key in adata.varm
            and group in adata.varm[weighted_lfc_varm_key].columns
        )
        ...
        if lfc_data_available and score_data_available:
            ...
            if weighted_lfc_data_available:
                logger.info(
                    f"Group-specific weighted mean log fold change data found for '{group}'"
                )
            else:
                logger.info(
                    f"Group-specific weighted mean log fold change data not available for '{group}'"
                )
