...
def plot_gene_expression(
    ...
    **kwargs :
    ...
    lfc_key, score_key = _infer_expression_keys(
        adata, run_id, lfc_key, score_key, strict=False
    )
    ...
    if condition1 is None or condition2 is None:
        ...
        if condition1 is None or condition2 is None:
            conditions = (
                _extract_conditions_from_key(lfc_key) if lfc_key is not None else None
            )
            if conditions:
    ...
    logger.info(
        f"Using fields for gene expression plot - lfc_key: {lfc_str}, score_key: {score_str}"
    )
    ...
    if layer is None and run_info is not None and "params" in run_info:
        ...
        if "layer" in params:
            ...
            if inferred_layer is not None and "fold_change" not in inferred_layer:
                ...
                logger.info(
                    f"Ignoring fold_change layer '{inferred_layer}' inferred from run, using adata.X instead"
                )

    # Extract fold change and score for the gene
    gene_lfc = (
        adata.var.loc[gene, lfc_key]
        if lfc_key is not None and lfc_key in adata.var
        else "Unknown"
    )
    gene_score = (
        adata.var.loc[gene, score_key]
        if score_key is not None and score_key in adata.var
        else "Unknown"
    )
    ...
    if basis not in adata.obsm:
        logger.warning(
            f"Basis '{basis}' not found in adata.obsm. Available bases: {list(adata.obsm.keys())}"
        )
    ...
    if (
        condition1_layer is None
        or condition2_layer is None
        or fold_change_layer is None
    ):
        missing = []
    ...
    if condition1_layer and condition1_layer in adata.layers:
        ...
        axs[0, 1].set_title(
            f"Imputed Expression ({condition1 or 'Condition 1'}): Not available"
        )
    ...
    if condition2_layer and condition2_layer in adata.layers:
        ...
        axs[1, 0].set_title(
            f"Imputed Expression ({condition2 or 'Condition 2'}): Not available"
        )
    ...
    if fold_change_layer and fold_change_layer in adata.layers:
        ...
        axs[1, 1].set_title("Log Fold Change: Not available")
