...
try:
    ...
    if (
        ...
        and "metaclass conflict" in str(e)
    ):
        error_msg = (
    ...
    def volcano_de(*args, **kwargs):
        raise ImportError(
            "Volcano plot functions unavailable due to scanpy compatibility issues. "
    ...
    def multi_volcano_da(*args, **kwargs):
        raise ImportError(
            "Multi volcano plot functions unavailable due to scanpy compatibility issues. "
            "Please update scanpy or use Python 3.9-3.11 instead of 3.12."
        )
...
try:
    ...
    def heatmap(*args, **kwargs):
        raise ImportError(
            "Heatmap functions unavailable due to scanpy compatibility issues. "
            "Please update scanpy or use Python 3.9-3.11 instead of 3.12."
        )

    def direction_barplot(*args, **kwargs):
        raise ImportError(
            "Direction barplot functions unavailable due to scanpy compatibility issues. "
            "Please update scanpy or use Python 3.9-3.11 instead of 3.12."
        )
...
try:
    ...
    if (
        ...
        logger.warning(
            f"Could not import gene expression plotting functions due to: {e}"
        )

    def plot_gene_expression(*args, **kwargs):
        raise ImportError(
            "Gene expression plotting functions unavailable due to scanpy compatibility issues. "
            "Please update scanpy or use Python 3.9-3.11 instead of 3.12."
        )
...
try:
    ...
    def embedding(*args, **kwargs):
        raise ImportError(
            "Embedding plotting function unavailable due to scanpy compatibility issues. "
            "Please update scanpy or use Python 3.9-3.11 instead of 3.12."
        )
...
try:
    ...
    class StringDBReport:
        ...
        def __init__(self, *args, **kwargs):
            raise ImportError(
                "StringDBReport is unavailable due to missing dependencies. "
