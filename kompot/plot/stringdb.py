...
class StringDBReport:
    ...
    def get_stringdb_image_url(
        self, additional_genes: Optional[List[str]] = None
    ) -> str:
    ...
    def get_resource_links(self, gene: str) -> Dict[str, str]:
        ...
        links["UniProt"] = (
            f"https://www.uniprot.org/uniprotkb?query={encoded_gene}+AND+organism_id:{self.species_id}"
        )

        # Species-specific resources
        if self.species_id == 9606:  # Human
            links["NCBI Gene"] = (
                f"https://www.ncbi.nlm.nih.gov/gene/?term={encoded_gene}+AND+human[orgn]"
            )
            ...
            links["MGI"] = (
                f"https://www.informatics.jax.org/quicksearch/summary?queryType=exactPhrase&query={encoded_gene}&submit=Quick%0D%0ASearch"
            )
            links["NCBI Gene"] = (
                f"https://www.ncbi.nlm.nih.gov/gene/?term={encoded_gene}+AND+mouse[orgn]"
            )
    ...
    def to_html(self, additional_genes: Optional[List[str]] = None) -> str:
        ...
        html_parts.append(
            f"<p><strong>Species:</strong> {species_name} (Taxonomy ID: {self.species_id})</p>"
        )
        ...
        if self.include_stringdb:
            ...
            html_parts.append(
                f'<p><a href="{stringdb_url}" target="_blank">View interactive network in StringDB</a></p>'
            )
            html_parts.append(
                f'<a href="{stringdb_url}" target="_blank"><img src="{image_url}" style="max-width:800px; border:1px solid #ddd;" alt="StringDB Network"></a>'
            )
        ...
        if self.include_resources:
            ...
            html_parts.append(
                '<tr><th style="width:15%; text-align:left;">Gene</th><th style="text-align:left;">Resource Links</th></tr>'
            )
            ...
            for gene in self.genes:
                ...
                html_parts.append(
                    f'<tr><td style="text-align:left;">{gene}</td><td style="text-align:left;">{link_html}</td></tr>'
                )
        ...
        if self.include_enrichment:
            try:
                ...
                html_parts.append(
                    f'<p><a href="{url}" target="_blank" style="font-weight: bold;">View interactive enrichment analysis on StringDB</a></p>'
                )
                ...
                for category, label, open_by_default in categories_to_try:
                    try:
                        ...
                        if enrichment_df is not None and not enrichment_df.empty:
                            ...
                            table_html = enrichment_df.head(num_rows)[
                                display_cols
                            ].to_html(
                                index=False,
                                escape=False,
    ...
    def get_functional_enrichment(
        ...
        if category not in self.annotation_categories:
            ...
            logger.warning(
                f"Invalid category '{category}'. Valid options are: {valid_cats}"
            )
        ...
        def process_enrichment_response(response, target_category):
            ...
            try:
                ...
                try:
                    ...
                    text = re.sub(
                        r":\s*([a-zA-Z][a-zA-Z0-9_]*)\s*([,}])", r': "\1"\2', text
                    )
                    ...
                    logger.debug(
                        f"Invalid JSON response from StringDB API: {str(e)} - {response.text[:100]}..."
                    )
                    return None
        ...
        try:
            ...
            logger.debug(
                f"Fetching enrichment data for category '{category}' (all categories approach)"
            )
            ...
            try:
                ...
                logger.debug(
                    "json5 module not available, using standard JSON parser with fixes"
                )
            ...
            if df is None or len(df) == 0:
                logger.debug(
                    "No results found using all-categories approach, trying category-specific request"
                )
                response = requests.post(
                    url, data=fallback_payload, headers=headers, timeout=30
                )
                df = process_enrichment_response(response, api_category)
                ...
                if (
                    df is None
                    or len(df) == 0
                    and category in ["Process", "Component", "Function"]
                ):
                    ...
                    response = requests.post(
                        url, data=temp_payload, headers=headers, timeout=30
                    )
                    df = process_enrichment_response(response, api_category)
            ...
            if df is not None and len(df) > 0:
                ...
                logger.warning(
                    f"No enrichment terms found for category '{category}' below FDR threshold {fdr_threshold}"
                )
                return None

        except Exception as e:
            logger.warning(
                f"Failed to fetch enrichment data for category '{category}': {e}"
            )
            return None
    ...
    def _compute_signal_and_strength(
        ...
        df["expected"] = (
            network_size * df["number_of_genes_in_background"]
        ) / total_proteins
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            df["signal"] = 2 / (
                1 / obs_exp_norm.clip(lower=1e-10)
                + 1 / neg_log_fdr_norm.clip(lower=1e-10)
            )
    ...
    def save_html(
        self, filename: str, additional_genes: Optional[List[str]] = None
    ) -> None:
    ...
    def get_json(self, additional_genes: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if self.include_resources:
            result["resources"] = {
                gene: self.get_resource_links(gene) for gene in self.genes
            }
