"""Plotting functions for expression smoothing results."""
...
def plot_smoothing(
    ...
    **kwargs,
    ...
    ----------
    adata : AnnData
        AnnData object with smoothing results.
    genes : list of str, optional
        Genes to plot. If *None*, selects top genes by mean smoothed value.
    n_top_genes : int
        Number of genes when ``genes`` is *None*. Max 8.
    basis : str
        Key in ``adata.obsm`` for 2-D coordinates.
    result_key : str
        Base key used in ``smooth_expression()``.
    ...
    **kwargs
    ...
    smoothed_key = f"{result_key}_{condition}_smoothed"
    std_key = f"{result_key}_{condition}_std"
    obs_var_key = f"{result_key}_{condition}_obs_variance"

    if smoothed_key not in adata.layers:
        raise ValueError(
            f"No smoothing layers found with prefix '{result_key}'. "
            f"Run kompot.smooth_expression() first or check result_key/condition."
        )

    # --- Cell mask (non-NaN rows in smoothed layer) ---
    imputed_full = adata.layers[smoothed_key]
    ...
    # --- Title ---
    if title is None:
        title = f"Expression Smoothing ({condition})"
...
def _detect_condition_label(adata, result_key: str) -> str:
    """Auto-detect the condition label from smoothing layer names."""
