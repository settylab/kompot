...
def embedding(
    ...
    **kwargs :
    ...
    if mgroups is not None:
        ...
        if "color" in kwargs and isinstance(kwargs["color"], (list, tuple, np.ndarray)):
            raise ValueError(
                "Cannot use multiple colors (list of color values) with mgroups parameter."
            )
        ...
        if titles is None:
            ...
            if len(mgroups_list) > 1:
                warnings.warn(
                    f"Provided titles list is too short (1) for the number of groups ({len(mgroups_list)}). Using default titles for the remaining groups."
                )
        ...
        if len(titles) < len(mgroups_list):
            ...
            if not (isinstance(original_title, str) and len(mgroups_list) > 1):
                warnings.warn(
                    f"Provided titles list is too short ({len(titles)}) for the number of groups ({len(mgroups_list)}). Using default titles for the remaining groups."
                )
            # Add generic titles for any missing
            if mgroups_dict:
                ...
                titles.extend(
                    [f"Group {i + 1}" for i in range(len(titles), len(mgroups_list))]
                )
    ...
    if basis_key not in adata.obsm:
        ...
        raise ValueError(
            f"Basis '{basis}' not found in adata.obsm. Available bases: {available_bases}"
        )
    ...
    if groups is None:
        ...
        for column, values in groups.items():
            if column not in adata.obs.columns:
                logger.warning(
                    f"Column '{column}' not found in adata.obs. Skipping this filter."
                )
                continue
        ...
        logger.info(
            f"Selected {n_selected:,} cells out of {adata.n_obs:,} total cells."
        )

        if n_selected == 0:
            logger.warning(
                "No cells match the filtering criteria. Check your group filters."
            )
            return None
    else:
