...
def get_dask_array(
    array_shape: tuple, chunk_size: Optional[int] = None
) -> Union["da.Array", None]:
    """
...
def get_disk_space(path: str) -> Tuple[str, int, str, int, str, int]:
    ...
    try:
        ...
        return (
            human_readable_size(fallback_total),
            fallback_total,
            human_readable_size(fallback_used),
            fallback_used,
            human_readable_size(fallback_free),
            fallback_free,
        )


def estimate_disk_requirement(
    n_cells: int, n_genes: int, dtype=np.float64
) -> Tuple[str, int]:
    """
...
def analyze_memory_requirements(
    ...
    for i, shape in enumerate(shapes):
        ...
        array_sizes.append(
            {"index": i, "shape": shape, "size_str": size_str, "size_bytes": size_bytes}
        )
    ...
    if not store_arrays_on_disk and status == "critical":
        logger.warning(
            ...
            + (
                "\n4. Install dask for better disk-backed storage (pip install dask)"
                if not DASK_AVAILABLE
                else ""
            )
            + "\n"
            + ("5" if not DASK_AVAILABLE else "4")
...
class DiskStorage:
    ...
    def __init__(
        ...
        if storage_dir is None:
            ...
            self.storage_dir = tempfile.mkdtemp(
                prefix=f"kompot_arrays_{self._instance_id}_"
            )
            self._temp_dir = True
            ...
            self.storage_dir = tempfile.mkdtemp(
                prefix=f"kompot_arrays_{self._instance_id}_", dir=storage_dir
            )
            self._temp_dir = True
        ...
        self.namespace = (
            namespace if namespace is not None else f"ns_{self._instance_id}"
        )
        ...
        if self.use_dask:
            logger.info(
                f"Initialized disk storage at {self.storage_dir} with dask support (namespace: {self.namespace})"
            )
        else:
            logger.info(
                f"Initialized disk storage at {self.storage_dir} (namespace: {self.namespace})"
            )
    ...
    def _check_disk_space(
        self, expected_size_bytes: Optional[int] = None, min_free_ratio: float = 0.1
    ):
        """
        ...
        if expected_size_bytes is not None:
            ...
            if free_bytes < required_bytes:
                ...
                usage_ratio = (
                    expected_size_bytes / free_bytes if free_bytes > 0 else float("inf")
                )
                ...
                if alternative_dirs:
                    error_msg += "\n"
                    error_msg += "3. Suggested directories with more space:\n"
                ...
                raise IOError(
                    f"Insufficient disk space: {free_h} available, {expected_h} required"
                )
    ...
    def cleanup(self):
        ...
        try:
            ...
            if (
                ...
                and self.storage_dir in DiskStorage._shared_dirs
                and DiskStorage._shared_dirs[self.storage_dir] == self._instance_id
            ):
                import shutil
        ...
        try:
            with lock:
                ...
                if hasattr(array, "compute") and callable(getattr(array, "compute")):
                    ...
                    logger.debug(
                        f"Computing dask array before saving to disk: {namespaced_key}"
                    )
                    array = array.compute()
                ...
                logger.debug(
                    f"Stored array '{key}' to disk: {self.array_registry[key]['size_human']}"
                )
    ...
    def load_array(self, key: str, lazy: bool = None) -> Union[np.ndarray, "da.Array"]:
        ...
        if use_lazy:
            ...
            logger.debug(
                f"Lazy loading array '{key}' from disk: {self.array_registry[key]['size_human']}"
            )
            # Use da.from_array instead of da.from_npy_stack for single .npy files
            return da.from_array(np.load(file_path), chunks="auto")
        else:
            logger.debug(
                f"Loading array '{key}' from disk: {self.array_registry[key]['size_human']}"
            )
    ...
    def as_dask_array(self, shape: tuple = None, dtype=np.float64) -> "da.Array":
        ...
        if not self.use_dask:
            raise ImportError(
                "Dask is not available. Install dask or set use_dask=False."
            )
        ...
        if shape is None:
            ...
            if all(len(info["shape"]) == 2 for info in self.array_registry.values()):
                ...
                raise ValueError(
                    "Cannot infer shape from stored arrays. Please provide shape explicitly."
                )
